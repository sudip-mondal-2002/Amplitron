...
if [ ! -d "$RTNEURAL_DIR" ]; then
    ...
    echo "RTNeural already present, verifying pinned commit..."
fi
cd "$RTNEURAL_DIR"
git fetch origin
git checkout "$RTNEURAL_VERSION"
cd - > /dev/null
echo "RTNeural is pinned to $RTNEURAL_VERSION."

# Inject a shim over RTNeural's bundled nlohmann/json so the project's
# authoritative copy (v3.11.3, fetched via CMake FetchContent) is the
# only version compiled into every translation unit.  Without this shim,
# both versions are included with different inline-namespace tags
# (json_abi_v3_11_1 vs json_abi_v3_11_3), causing an "ambiguous
# json_pointer" hard error.
RTNEURAL_JSON_SHIM="$RTNEURAL_DIR/modules/json/json.hpp"
mkdir -p "$(dirname "$RTNEURAL_JSON_SHIM")"
cat > "$RTNEURAL_JSON_SHIM" << 'SHIM_EOF'
// RTNeural json.hpp shim — auto-generated by scripts/setup_dependencies.sh
...
// conflicts. Do not edit; re-run setup_dependencies.sh to regenerate.
#ifndef INCLUDE_NLOHMANN_JSON_HPP_
#define JSON_SKIP_LIBRARY_VERSION_CHECK
#endif
#include <nlohmann/json.hpp>
SHIM_EOF
echo "RTNeural json.hpp shim written."
