...
extern "C" EMSCRIPTEN_KEEPALIVE void on_midi_cc(int channel, int cc,
                                                int value) {
  ...
  emscripten_log(EM_LOG_DEBUG, "[MIDI] CC %d = %d on channel %d", cc, value,
                 channel);
}

extern "C" EMSCRIPTEN_KEEPALIVE void on_midi_device_connected(
...
extern "C" EMSCRIPTEN_KEEPALIVE const char* export_preset_json() {
  ...
  try {
    exported_json =
        Amplitron::PresetManager::preset_to_json_string(*g_engine_ptr);
  ...
  try {
    ...
    if (!Amplitron::from_json_ext(json_str, preset)) {
      emscripten_log(EM_LOG_ERROR,
                     "[Web] Failed to parse imported preset JSON.");
      return;
    }
    ...
    if (preset.routing == "graph") {
      ...
      if (Amplitron::PresetManager::graph_from_json(repackaged_json,
                                                    g_engine_ptr->graph())) {
...
extern "C" EMSCRIPTEN_KEEPALIVE void on_canvas_touch_gesture(float dx, float dy,
                                                             float dscale,
                                                             float local_x,
                                                             float local_y) {
  ...
  if (dscale != 0.0f) {
    ...
    ui.target_scrolling.x =
        local_x - (local_x - ui.target_scrolling.x) * actual_factor;
    ui.target_scrolling.y =
        local_y - (local_y - ui.target_scrolling.y) * actual_factor;
...
extern "C" EMSCRIPTEN_KEEPALIVE int trigger_add_splitter_node() {
  ...
  auto cmd = std::make_unique<Amplitron::AddGraphNodeCommand>(
      g_gui->audio_engine(), "Splitter", Amplitron::NodeRoutingType::Splitter,
      nullptr, ImVec2(0, 0));
  auto* raw = cmd.get();
...
extern "C" EMSCRIPTEN_KEEPALIVE int trigger_add_link(int src_pin, int dst_pin) {
  if (!g_gui) return -1;
  auto cmd = std::make_unique<Amplitron::AddGraphLinkCommand>(
      g_gui->audio_engine(), src_pin, dst_pin);
  auto* raw = cmd.get();
  g_gui->command_history().execute(std::move(cmd));
  return raw->was_successful ? raw->link.id : -1;
}

extern "C" EMSCRIPTEN_KEEPALIVE int get_node_output_pin_by_index(
    int node_index, int pin_index) {
  if (!g_gui) return -1;
  const auto& nodes = g_gui->audio_engine().graph().get_nodes();
  if (node_index < 0 || node_index >= static_cast<int>(nodes.size())) return -1;
  const auto& node = nodes[node_index];
  if (pin_index < 0 ||
      pin_index >= static_cast<int>(node.output_pin_ids.size()))
    return -1;
  return node.output_pin_ids[pin_index];
}

extern "C" EMSCRIPTEN_KEEPALIVE int get_node_input_pin_by_index(int node_index,
                                                                int pin_index) {
  if (!g_gui) return -1;
  const auto& nodes = g_gui->audio_engine().graph().get_nodes();
  if (node_index < 0 || node_index >= static_cast<int>(nodes.size())) return -1;
  const auto& node = nodes[node_index];
  if (pin_index < 0 || pin_index >= static_cast<int>(node.input_pin_ids.size()))
    return -1;
...
extern "C" EMSCRIPTEN_KEEPALIVE bool trigger_delete_last_node() {
  ...
  // Walk backwards to find the last deletable node (mirrors GUI rules: Input
  // and Amp Sim are protected)
  for (int i = static_cast<int>(nodes.size()) - 1; i >= 0; --i) {
    const auto& node = nodes[i];
    if (node.name == "Input" || node.name == "Amp Sim") continue;

    auto& ui_positions =
        Amplitron::GuiGraphState::get_instance().node_positions;
    ImVec2 pos(0, 0);
    ...
    // Note: node_positions.erase is handled inside
    // RemoveGraphNodeCommand::execute()
    return true;
  }
...
int main(int argc, char* argv[]) {
  ...
  // If the parser didn't already trigger an exit (like --help) validate the
  // preset
  if (!cli_opts.exit_early && cli_opts.preset_path.empty()) {
    std::cerr
        << "Error: Strict headless build requires a --preset <path> argument."
        << std::endl;
  ...
  if (cli_opts.exit_early) {
    std::cout << "[CLI]Application exited early :" << cli_opts.exit_reason
              << std::endl;
  ...
  if (cli_opts.is_headless) {
    ...
    if (!session.presets().load_preset(cli_opts.preset_path, engine, nullptr)) {
      std::cerr << "Fatal Error: Could not load preset for headless mode."
                << std::endl;
      engine.shutdown();
      return 1;
    }
    ...
    if (!cli_opts.input_device.empty()) {
      ...
      if (match_indices.empty()) {
        ...
        std::cout << "Input routed to: " << devices[match_indices[0]].name
                  << std::endl;
      } else {
        ...
        std::cerr << "Auto-selecting the first match: "
                  << devices[match_indices[0]].name << std::endl;
        engine.set_input_device(devices[match_indices[0]].index);
      }
    }
    ...
    if (!cli_opts.output_device.empty()) {
      ...
      if (match_indices.empty()) {
        ...
        std::cout << "Output routed to: " << devices[match_indices[0]].name
                  << std::endl;
      } else {
        std::cerr << "Warning: Ambiguous output name '"
                  << cli_opts.output_device
                  << "'. Multiple matches found:" << std::endl;
        for (int idx : match_indices) {
          std::cerr << " -- " << devices[idx].name << std::endl;
        }
  ...
  if (cli_opts.is_headless) {
    ...
    std::cout << "Commands: chain, gain <val>, bypass <idx>, enable <idx>, "
                 "telemetry <on/off>"
              << std::endl;
      ...
      for (const std::string& line : pending_commands) {
        if (line.find("gain ") == 0) {
          ...
          try {
            ...
            // push_effect_enabled expects a float(>0.5 is enabled, <0.5 is
            // bypassed)
          ...
          std::cout << ">> Telemetry muted. Type 'telemetry on' to resume."
                    << std::endl;
          ...
          std::cout << ">> Unknown command. Available: gain <val>, bypass "
                       "<idx>, enable "
      ...
      if (++loop_counter >= 100) {
        if (show_telemetry.load(std::memory_order_relaxed)) {
          ...
          std::string dashboard =
              "\n========================================\n";
          dashboard += "Active Buffer Size: " +
                       std::to_string(engine.get_buffer_size()) +
                       " samples @ " +
                       std::to_string(engine.get_sample_rate()) + "Hz\n";
