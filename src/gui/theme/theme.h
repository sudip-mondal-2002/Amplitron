...
constexpr ImU32 BG_SURFACE =
    IM_COL32(50, 46, 38, 255);  // #322e26  elevated surfaces
constexpr ImU32 BG_GRILLE =
    IM_COL32(26, 18, 16, 255);  // #1a1210  speaker grille

// Metal / borders
constexpr ImU32 BORDER_DARK =
    IM_COL32(58, 54, 48, 255);  // #3a3630  subtle border
constexpr ImU32 BORDER_MID =
    IM_COL32(68, 68, 68, 255);  // #444444  medium border
constexpr ImU32 BORDER_LIGHT =
    IM_COL32(85, 85, 85, 255);  // #555555  bright border
constexpr ImU32 METAL_STRIP =
    IM_COL32(61, 56, 48, 255);  // #3d3830  brushed metal
...
constexpr ImU32 SWITCH_ACTIVE =
    IM_COL32(72, 65, 42, 255);  // gold-tinted = enabled
constexpr ImU32 SWITCH_IDLE = IM_COL32(28, 26, 22, 255);  // dark = bypassed
constexpr ImU32 LED_OFF = IM_COL32(36, 34, 30, 255);
constexpr ImU32 PEDAL_BYPASS_OVERLAY =
    IM_COL32(0, 0, 0, 90);  // dim overlay when bypassed
...
constexpr float KNOB_SPACING_X =
    85.0f;  // horizontal distance between knob columns
...
constexpr float SWITCH_BOTTOM_OFFSET =
    55.0f;  // footswitch distance from pedal bottom
...
// --- Effect Color Scheme (SOLID: Open/Closed — extend by adding entries, not
// modifying) ---
...
inline const EffectColorEntry* get_effect_color(const char* effect_name) {
  ...
  static const EffectColorEntry table[] = {
      {"Distortion", ImVec4(0.45f, 0.14f, 0.10f, 1.0f),
       ImVec4(1.0f, 0.30f, 0.20f, 1.0f)},
      {"Overdrive", ImVec4(0.45f, 0.34f, 0.12f, 1.0f),
       ImVec4(0.95f, 0.75f, 0.20f, 1.0f)},
      {"Delay", ImVec4(0.12f, 0.22f, 0.38f, 1.0f),
       ImVec4(0.35f, 0.60f, 0.95f, 1.0f)},
      {"Reverb", ImVec4(0.14f, 0.28f, 0.36f, 1.0f),
       ImVec4(0.25f, 0.72f, 0.85f, 1.0f)},
      {"Looper", ImVec4(0.16f, 0.16f, 0.22f, 1.0f),
       ImVec4(0.78f, 0.66f, 0.29f, 1.0f)},
      {"Chorus", ImVec4(0.26f, 0.14f, 0.38f, 1.0f),
       ImVec4(0.65f, 0.35f, 0.95f, 1.0f)},
      {"Phaser", ImVec4(0.22f, 0.10f, 0.34f, 1.0f),
       ImVec4(0.80f, 0.25f, 0.90f, 1.0f)},
      {"Flanger", ImVec4(0.14f, 0.20f, 0.36f, 1.0f),
       ImVec4(0.30f, 0.70f, 1.00f, 1.0f)},
      {"Equalizer", ImVec4(0.14f, 0.32f, 0.18f, 1.0f),
       ImVec4(0.25f, 0.90f, 0.40f, 1.0f)},
      {"Noise Gate", ImVec4(0.20f, 0.19f, 0.22f, 1.0f),
       ImVec4(0.70f, 0.70f, 0.80f, 1.0f)},
      {"Compressor", ImVec4(0.34f, 0.26f, 0.14f, 1.0f),
       ImVec4(0.95f, 0.65f, 0.25f, 1.0f)},
      {"MultiBand Compressor", ImVec4(0.24f, 0.30f, 0.28f, 1.0f),
       ImVec4(0.40f, 0.85f, 0.70f, 1.0f)},
      {"Cabinet", ImVec4(0.26f, 0.18f, 0.10f, 1.0f),
       ImVec4(0.85f, 0.55f, 0.30f, 1.0f)},
      {"Octaver", ImVec4(0.36f, 0.14f, 0.28f, 1.0f),
       ImVec4(0.90f, 0.35f, 0.60f, 1.0f)},
      {"Pitch Shifter", ImVec4(0.32f, 0.12f, 0.32f, 1.0f),
       ImVec4(0.85f, 0.40f, 0.85f, 1.0f)},
      {"Tuner", ImVec4(0.12f, 0.24f, 0.30f, 1.0f),
       ImVec4(0.40f, 0.85f, 0.95f, 1.0f)},
  };
  for (const auto& entry : table) {
    if (std::strcmp(effect_name, entry.name) == 0) return &entry;
  }
  // Default fallback
  static const EffectColorEntry fallback = {"Default",
                                            ImVec4(0.20f, 0.19f, 0.17f, 1.0f),
                                            ImVec4(0.78f, 0.66f, 0.29f, 1.0f)};
  return &fallback;
}
