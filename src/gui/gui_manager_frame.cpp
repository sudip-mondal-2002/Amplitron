...
void GuiManager::render_master_controls() {
    ...
    if (ImGui::Button("Tap")) {
        ...
        if (!s_global_tap_times.empty()) {
            auto diff =
                std::chrono::duration_cast<std::chrono::seconds>(now - s_global_tap_times.back())
                    .count();
            if (diff >= 4) {
        ...
        if (s_global_tap_times.size() >= 2) {
            ...
            for (size_t idx = 1; idx < s_global_tap_times.size(); ++idx) {
                double ms = std::chrono::duration<double, std::milli>(s_global_tap_times[idx] -
                                                                      s_global_tap_times[idx - 1])
                                .count();
