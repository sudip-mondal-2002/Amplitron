...
TunerProps GuiManager::build_tuner_props() {
  ...
  p.on_mute_changed = [t](bool mute) {
    t->params()[0].value = mute ? 1.0f : 0.0f;
  };
...
SettingsProps GuiManager::build_settings_props() {
  ...
  p.latency_ms =
      (p.sample_rate > 0) ? (1000.0f * p.buffer_size / p.sample_rate) : 0.0f;
  ...
  p.on_auto_buf_changed = [this](bool b) {
    engine_.set_auto_buffer_enabled(b);
  };
...
void GuiManager::recallSnapshotFromSlot(int slot) {
  ...
  command_history_.execute(std::make_unique<RecallSnapshotCommand>(
      engine_, before.effects, before.input_gain, before.output_gain,
...
bool GuiManager::run_frame() {
  ...
  // ── Keyboard shortcuts ──
  {
    ...
    static const ImGuiKey digit_keys[4] = {ImGuiKey_1, ImGuiKey_2, ImGuiKey_3,
                                           ImGuiKey_4};
    for (int i = 0; i < 4; ++i) {
  ...
  float analyzer_reserved_h =
      is_fullscreen ? 0.0f : gui_analyzer_.analyzer_reserved_height();
  ...
  if (toast_timer_ > 0.0f) {
    ...
    ImGuiIO& io = ImGui::GetIO();
    ImVec2 toast_pos =
        ImVec2(io.DisplaySize.x - 20.0f, io.DisplaySize.y - 20.0f);
...
void GuiManager::render_master_controls() {
  smoothed_input_level_ +=
      (engine_.get_input_level() - smoothed_input_level_) * 0.3f;
  smoothed_output_level_ +=
      (engine_.get_output_level() - smoothed_output_level_) * 0.3f;

  ImGui::BeginChild("MasterControls", ImVec2(0, 110), true,
    ...
    if (s_global_tap_times.size() >= 2) {
      ...
      for (size_t idx = 1; idx < s_global_tap_times.size(); ++idx) {
        double ms = std::chrono::duration<double, std::milli>(
                        s_global_tap_times[idx] - s_global_tap_times[idx - 1])
                        .count();
