...
extern "C" EMSCRIPTEN_KEEPALIVE void amplitron_on_file_uploaded(
    const char* path) {
  if (path) {
  ...
  EM_ASM(
      {
        ...
        inp.onchange = function(ev) {
          ...
          reader.onload = function(re) {
            ...
            Module.ccall('amplitron_on_file_uploaded',
                         null, ['string'], [path]);
          };
...
#include <string>
namespace Amplitron {
std::string show_save_dialog(const std::string&, const std::string&,
                             const std::string&) {
  return "";
}
