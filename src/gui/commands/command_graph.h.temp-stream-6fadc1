...
struct AddGraphNodeCommand : public Command {
  ...
  AddGraphNodeCommand(IAudioEngine& engine, const std::string& name,
                      EffectType type, std::shared_ptr<Effect> pedal,
                      ImVec2 pos, int num_inputs = 0)
      : engine_(engine),
        name(name),
        type(type),
        pedal(pedal),
  ...
  bool execute() override {
    ...
    if (position.x != 0.0f || position.y != 0.0f) {
      GuiGraphState::get_instance().node_positions[node_id] = {position, false,
                                                               ImVec2(0, 0)};
    }
...
struct RemoveGraphNodeCommand : public Command {
  ...
  std::vector<RemovedPinInfo>
      auto_removed_pins;  // cache for dynamically removed pins on other nodes
  std::vector<RemovedPinInfo> auto_removed_out_pins;

  RemoveGraphNodeCommand(IAudioEngine& engine, NodeId id, EffectType t,
                         ImVec2 pos)
  ...
  bool execute() override {
    ...
    for (const auto& link : engine_.graph().get_links()) {
      if (std::find(cached_node.input_pin_ids.begin(),
                    cached_node.input_pin_ids.end(),
                    link.dest_pin_id) != cached_node.input_pin_ids.end() ||
          std::find(cached_node.output_pin_ids.begin(),
                    cached_node.output_pin_ids.end(),
                    link.source_pin_id) != cached_node.output_pin_ids.end()) {
    ...
    for (const auto& link : severed_links) {
      ...
      if (dest_node_id != -1) {
        ...
        if (n && (n->routing_type == NodeRoutingType::Mixer ||
                  n->routing_type == NodeRoutingType::MergeSum)) {
          if (n->input_pin_ids.size() > 2) {
            ...
            for (size_t i = 0; i < n->input_pin_ids.size(); ++i) {
              if (n->input_pin_ids[i] == link.dest_pin_id) {
                idx = i;
                gain = (i < n->input_gains.size()) ? n->input_gains[i] : 1.0f;
                break;
              }
            }
            if (engine_.graph().remove_input_pin(dest_node_id,
                                                 link.dest_pin_id)) {
              auto_removed_pins.push_back(
                  {dest_node_id, link.dest_pin_id, idx, gain});
            }
          }
        }
      }
      ...
      int source_node_id =
          engine_.graph().get_node_from_pin(link.source_pin_id);
      if (source_node_id != -1) {
        ...
        if (n && n->routing_type == NodeRoutingType::Splitter) {
          if (n->output_pin_ids.size() > 2) {
            ...
            if (engine_.graph().remove_output_pin(source_node_id,
                                                  link.source_pin_id)) {
              auto_removed_out_pins.push_back(
                  {source_node_id, link.source_pin_id, idx, 1.0f});
            }
          }
        }
      }
  ...
  void undo() override {
    ...
    for (auto it = auto_removed_pins.rbegin(); it != auto_removed_pins.rend();
         ++it) {
      engine_.graph().restore_input_pin(it->node_id, it->pin_id, it->index,
                                        it->gain);
    }
    for (auto it = auto_removed_out_pins.rbegin();
         it != auto_removed_out_pins.rend(); ++it) {
...
struct AddGraphLinkCommand : public Command {
  ...
  bool execute() override {
    if (link.id == -1) {
      ...
      for (const auto& l : engine_.graph().get_links()) {
        if (l.source_pin_id == link.source_pin_id &&
            l.dest_pin_id == link.dest_pin_id) {
          already_exists = true;
          break;
        }
      }
      ...
      if (was_successful) {
        ...
        if (dest_node_id != -1) {
          ...
          if (node && (node->routing_type == NodeRoutingType::Mixer ||
            ...
            if (occupied_count == node->input_pin_ids.size()) {
              if (engine_.graph().add_input_pin(dest_node_id)) {
                const DSPNode* updated_node =
                    engine_.graph().find_node(dest_node_id);
      ...
      if (auto_added_pin_node_id != -1) {
        engine_.graph().restore_input_pin(
            auto_added_pin_node_id, auto_added_pin_id, auto_added_pin_index,
            auto_added_pin_gain);
      }
      if (auto_added_out_pin_node_id != -1) {
        engine_.graph().restore_output_pin(auto_added_out_pin_node_id,
                                           auto_added_out_pin_id,
                                           auto_added_out_pin_index);
      }
      engine_.graph().restore_link(link);
    }
  ...
  void undo() override {
    if (was_successful) {
      ...
      if (auto_added_pin_node_id != -1) {
        engine_.graph().remove_input_pin(auto_added_pin_node_id,
                                         auto_added_pin_id);
      }
      if (auto_added_out_pin_node_id != -1) {
        engine_.graph().remove_output_pin(auto_added_out_pin_node_id,
                                          auto_added_out_pin_id);
      }
      engine_.commit_graph_changes();
    }
...
struct RemoveGraphLinkCommand : public Command {
          ...
          if (node->input_pin_ids.size() > 2) {
            for (size_t i = 0; i < node->input_pin_ids.size(); ++i) {
              if (node->input_pin_ids[i] == link.dest_pin_id) {
                ...
                auto_removed_pin_gain = (i < node->input_gains.size())
                                            ? node->input_gains[i]
                                            : 1.0f;
                break;
              }
  ...
  void undo() override {
    if (auto_removed_pin_node_id != -1) {
      engine_.graph().restore_input_pin(
          auto_removed_pin_node_id, auto_removed_pin_id, auto_removed_pin_index,
          auto_removed_pin_gain);
    }
    if (auto_removed_out_pin_node_id != -1) {
      engine_.graph().restore_output_pin(auto_removed_out_pin_node_id,
                                         auto_removed_out_pin_id,
                                         auto_removed_out_pin_index);
    }
    engine_.graph().restore_link(link);
    engine_.commit_graph_changes();
  }
...
struct MoveGraphNodeCommand : public Command {
  ...
  bool execute() override {
    ...
    if (it->second.position.x == new_pos.x &&
        it->second.position.y == new_pos.y) {
      return false;
    }
