...
class AddEffectCommand : public Command {
 public:
  ...
  /** @brief Append the effect to the engine's chain (before the amp if
   * present). */
...
class RemoveEffectCommand : public Command {
 public:
   ...
   * @param index   Chain index of the effect to remove (captured at
   * construction).
   */
  RemoveEffectCommand(IAudioEngine& engine, int index)
