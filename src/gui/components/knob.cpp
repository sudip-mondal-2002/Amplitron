...
void KnobComponent::render(const char* imgui_id, const KnobProps& props,
                           float zoom, ImVec2 center) {
  ...
  if (is_active && s_active_knob_id == imgui_id) {
    ...
    if (mdx != 0.0f || mdy != 0.0f) {
      ...
      float new_val =
          clamp(props.value + value_delta, props.min_val, props.max_val);
  ...
  if (is_hovered && std::fabs(ImGui::GetIO().MouseWheel) > 0.0f) {
    ...
    float new_val = clamp(props.value + ImGui::GetIO().MouseWheel * step,
                          props.min_val, props.max_val);
    if (new_val != old_val) {
  ...
  if (ImGui::BeginPopup(popup_id)) {
    ...
    ImGui::SliderFloat("##edit", &slider_val, props.min_val, props.max_val,
                       "%.2f");
    ...
    if (ImGui::IsItemDeactivatedAfterEdit() && s_active_popup_id == popup_id) {
      if (props.value != s_popup_param_value_before_edit &&
          props.on_value_committed) {
    ...
    if (ImGui::Button("Reset")) {
      ...
      if (new_val != old_val) {
        if (props.on_value_changed) props.on_value_changed(new_val);
        if (props.on_value_committed)
          props.on_value_committed(old_val, new_val);
      }
  ...
  for (int s = 0; s < segments; ++s) {
    ...
    ImU32 seg_color = filled ? ImGui::ColorConvertFloat4ToU32(props.led_color)
  ...
  if (props.is_learning) {
    ...
    dl->AddCircle(center, knob_radius + 4.0f * zoom, outline_col, 0,
                  3.0f * zoom);
  }
  ...
  if (is_hovered || is_active) {
    ...
    std::string min_str =
        Theme::formatParameterValue(props.min_val, props.unit);
    std::string max_str =
        Theme::formatParameterValue(props.max_val, props.unit);
