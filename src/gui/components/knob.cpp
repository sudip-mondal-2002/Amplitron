...
void KnobComponent::render(const char* imgui_id, const KnobProps& props, float zoom, ImVec2 center) {
    ...
    if (is_hovered || is_active) {
        if (!props.tooltip.empty() || !props.midi_info.empty()) {
            if (!props.tooltip.empty() && !props.midi_info.empty()) {
                ImGui::SetTooltip("%s\n\n%s", props.tooltip.c_str(), props.midi_info.c_str());
            } else if (!props.tooltip.empty()) {
                ImGui::SetTooltip("%s", props.tooltip.c_str());
            } else {
                ImGui::SetTooltip("%s", props.midi_info.c_str());
            }
        }
    }

    // Parameter names and values text
    std::string pname_with_unit = props.name;
    if (!props.unit.empty()) {
        pname_with_unit += " (" + props.unit + ")";
    }
    ImVec2 text_size = ImGui::CalcTextSize(pname_with_unit.c_str());
    // Increased padding between knob and label (from 8 to 11)
    float label_y = center.y + knob_radius + 11.0f * zoom;
    ImGui::SetCursorScreenPos(ImVec2(center.x - text_size.x * 0.5f, label_y));
    ...
    ImGui::SetCursorScreenPos(ImVec2(center.x - val_size.x * 0.5f, val_y));
