...
void LedComponent::render(const char* imgui_id, const LedProps& props,
  ...
  if (props.blink) {
    ...
    if (is_on) {
      base_color = ImGui::ColorConvertFloat4ToU32(ImVec4(
          props.led_color.x, props.led_color.y, props.led_color.z, osc.w));
    }
  } else if (is_on) {
  ...
  if (is_on) {
    int glow_alpha =
        props.blink ? static_cast<int>(Theme::RecBlink(props.blink_time).w * 40)
                    : 40;
    dl->AddCircleFilled(
        center, glow_radius,
        IM_COL32(static_cast<int>(props.led_color.x * 255),
                 static_cast<int>(props.led_color.y * 255),
                 static_cast<int>(props.led_color.z * 255), glow_alpha));
  }

  if (props.tooltip) {
    ImGui::SetCursorScreenPos(
        ImVec2(center.x - 10.0f * zoom, center.y - 10.0f * zoom));
