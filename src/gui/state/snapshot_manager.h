 ...
 * and all parameter values — using the LoadPresetCommand::EffectSnapshot
 * pattern so that recall integrates cleanly with the undo/redo system.
 */
class SnapshotManager {
 public:
   ...
   *
   * Mirrors the data captured by LoadPresetCommand so that
   * RecallSnapshotCommand can restore it using the same atomic engine swap.
   */
  ...
  /** @brief Return a pointer to the stored snapshot, or nullptr if the slot is
   * empty. */
