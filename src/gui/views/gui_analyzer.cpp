...
// ─────────────────────────────────────────────────────────────────────────────
void GuiAnalyzer::render_vu_bar(const char* id, const char* label,
                                float rms_level, float peak_hold,
                                bool clip_active, float clip_flash,
                                ImU32 base_color, ImU32 peak_color) {
  ImGui::PushID(id);
  ImGui::TextUnformatted(label);
  ImGui::SameLine();
  float db_value =
      (rms_level > 0.0001f) ? (20.0f * std::log10(rms_level)) : -96.0f;
  ...
  dl->AddRectFilled(pos, ImVec2(pos.x + width, pos.y + height), bg_col,
                    Theme::ROUNDING_SM);

  float rms_fill = clamp(rms_level, 0.0f, 1.0f) * width;
  dl->AddRectFilled(pos, ImVec2(pos.x + rms_fill, pos.y + height), base_color,
                    Theme::ROUNDING_SM);

  float peak_x = pos.x + clamp(peak_hold, 0.0f, 1.0f) * width;
  dl->AddLine(ImVec2(peak_x, pos.y - 1.0f),
              ImVec2(peak_x, pos.y + height + 1.0f), peak_color, 2.0f);

  if (clip_active || clip_flash > 0.01f)
    dl->AddText(ImVec2(pos.x + width - 32.0f, pos.y - 1.0f),
                IM_COL32(255, 90, 90, 255), "CLIP");
...
// ─────────────────────────────────────────────────────────────────────────────
void GuiAnalyzer::draw_spectrum(ImDrawList* dl, const ImVec2& pos,
  ...
  for (float db : ref_lines) {
    ...
    dl->AddLine(ImVec2(pos.x, y), ImVec2(pmax.x, y), IM_COL32(58, 64, 76, 180),
                1.0f);
  }
  ...
  const auto draw_set = [&](const std::array<float, BARS>& bars,
                            const std::array<float, BARS>& peaks, ImU32 bar_col,
                            float width_scale) {
    for (int i = 0; i < BARS; ++i) {
      ...
      dl->AddRectFilled(ImVec2(center - half, y), ImVec2(center + half, pmax.y),
                        bar_col, 1.5f);

      const float peak_t =
          (clamp(peaks[i], kDisplayMinDb, kDisplayMaxDb) - kDisplayMinDb) /
          (kDisplayMaxDb - kDisplayMinDb);
      const float py = pmax.y - peak_t * size.y;
      dl->AddLine(ImVec2(center - half, py), ImVec2(center + half, py),
                  peak_col, 1.0f);
    }
  };
  ...
  for (float hz : ticks) {
    ...
    dl->AddLine(ImVec2(x, pos.y), ImVec2(x, pmax.y), IM_COL32(52, 58, 72, 180),
                1.0f);
  }
}
...
void GuiAnalyzer::render() {
  ...
  ImGui::BeginChild("AnalyzerPanel", ImVec2(0, panel_h), true,
  ...
  render_vu_bar("input_vu", "INPUT RMS", p.smoothed_input_rms,
                p.input_peak_hold, p.input_clip_active, p.input_clip_flash,
                IM_COL32(60, 200, 110, 230), IM_COL32(255, 230, 120, 255));
  ImGui::NextColumn();
  render_vu_bar("output_vu", "OUTPUT RMS", p.smoothed_output_rms,
                p.output_peak_hold, p.output_clip_active, p.output_clip_flash,
