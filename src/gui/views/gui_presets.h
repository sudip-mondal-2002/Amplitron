...
class GuiPresets {
 public:
  /**
   * @brief Construct a GuiPresets helper for the given audio engine and undo
   * history.
   ...
   */
  GuiPresets(IAudioEngine& engine, CommandHistory& history,
  ...
  /** @brief Return true if the current engine state differs from the last saved
   * preset. */
  bool is_dirty() const;
  ...
  /** @brief Serialise the current engine state to a JSON string for clipboard
   * export. */
