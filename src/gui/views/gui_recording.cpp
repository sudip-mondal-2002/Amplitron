...
void GuiRecording::render() {
  ...
  ImGui::BeginChild(
      "RecordingPanel", ImVec2(0, panel_height), true,
  ...
  if (p.is_recording) {
    ...
    if (p.is_paused) {
      ...
      if (ImGui::Button("RESUME", ImVec2(80 * font_scale, 0)) && p.on_resume)
        p.on_resume();
      ...
      if (ImGui::Button("PAUSE", ImVec2(80 * font_scale, 0)) && p.on_pause)
        p.on_pause();
      ImGui::PopStyleColor(2);
    }
    ...
    ImGui::TextColored(ImVec4(0.9f, 0.9f, 0.9f, 1.0f), "  %02d:%02d.%d", mins,
                       secs, ms);
    ...
    ImU32 wave_color =
        p.is_paused ? IM_COL32(180, 160, 50, 200) : IM_COL32(200, 80, 60, 220);
    ImU32 wave_color_bright =
        p.is_paused ? IM_COL32(220, 200, 80, 255) : IM_COL32(255, 100, 70, 255);
    ...
    if (p.waveform_buf && p.waveform_size > 0) {
      ...
      for (int i = 0; i < num_bars; ++i) {
        ...
        draw->AddLine(ImVec2(x, center_y - bar_h), ImVec2(x, center_y + bar_h),
                      col);
      }
    }

    draw->AddRect(wave_pos, ImVec2(wave_pos.x + wave_w, wave_pos.y + wave_h),
    ...
    float content_w = ImGui::CalcTextSize("Recording complete").x +
                      ImGui::CalcTextSize("  999.9 s  |  ").x + 100.0f + 80.0f +
    ...
    if (ImGui::Button("Discard", ImVec2(80 * font_scale, 0))) {
      ...
      set_state(
          [](RecordingState& st) { st.status_msg = "Recording discarded."; });
    }
    ImGui::PopStyleColor(2);
    ...
    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f),
                       "  Ready to record  |  WAV 16-bit %d Hz", p.sample_rate);
  }

  ImGui::EndChild();
}

void GuiRecording::render_save_dialog(
    std::function<void(const std::string& dest)> on_save_done) {
  if (!state_.needs_save) return;
  state_.needs_save = false;

  std::string dest = show_save_dialog("recording.wav", "WAV Audio", "wav");
  if (!dest.empty() && on_save_done) on_save_done(dest);
}
