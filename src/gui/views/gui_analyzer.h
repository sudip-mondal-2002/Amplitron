...
class GuiAnalyzer : public UIComponent<AnalyzerProps> {
 ...
 private:
  void render_vu_bar(const char* id, const char* label, float rms_level,
                     float peak_hold, bool clip_active, float clip_flash,
                     ImU32 base_color, ImU32 peak_color);

  void draw_spectrum(ImDrawList* dl, const ImVec2& pos,
