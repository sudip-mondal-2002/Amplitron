...
void GuiTuner::render(bool& show) {
  ...
  if (p.has_signal && p.note_idx >= 0 && p.note_name_fn) {
    ...
    std::snprintf(note_buf, sizeof(note_buf), "%s%d",
                  p.note_name_fn(p.note_idx), p.octave);
    ...
    dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * scale, note_pos,
    ...
    dl->AddRectFilled(ImVec2(needle_x - 4, bar_pos.y - 3),
                      ImVec2(needle_x + 4, bar_pos.y + bar_h + 3), needle_col,
                      3.0f);
    ...
    dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * scale, pos,
                Theme::TEXT_DIM, dash);
    ImGui::Dummy(ImVec2(0, ImGui::GetFontSize() * scale + 8));
