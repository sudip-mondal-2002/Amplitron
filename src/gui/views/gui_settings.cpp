...
void GuiSettings::render(bool& show) {
  ...
  if (ImGui::Combo("Buffer Size", &current_idx, buf_labels, 5)) {
    if (p.on_buffer_size_changed)
      p.on_buffer_size_changed(buf_sizes[current_idx]);
  }
  ...
  if (p.suggested_buf != p.buffer_size) {
    ...
    std::snprintf(suggest_label, sizeof(suggest_label), "Switch to %d",
                  p.suggested_buf);
  ...
  ImGui::TextWrapped(
      "Select your USB guitar cable or audio interface. USB devices are "
      "highlighted with [USB].");
  ...
  for (const auto& dev : p.output_devices) {
    ...
    std::string label =
        dev.name + (dev.is_usb ? "  [USB - not recommended]" : "");
    if (dev.is_usb)
      ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.6f, 0.6f, 0.6f, 0.7f));
    if (ImGui::Selectable(label.c_str(), is_selected) &&
        p.on_output_device_changed)
  ...
  ImGui::Separator();
  ImGui::TextDisabled(
      "MIDI settings are managed in a separate window (Utilities > MIDI "
      "Settings).");
