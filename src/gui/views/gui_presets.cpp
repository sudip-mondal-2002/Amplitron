...
extern "C" {
EMSCRIPTEN_KEEPALIVE void load_preset_callback(uintptr_t gui_ptr,
                                               const char* path) {
  if (gui_ptr && path) {
...
static bool equal_effect_data(const PresetData::EffectData& a,
  ...
  if (a.type != b.type || a.enabled != b.enabled || a.mix != b.mix)
    return false;
...
static bool equal_preset_data(const PresetData& a, const PresetData& b) {
  if (a.input_gain != b.input_gain || a.output_gain != b.output_gain)
    return false;
...
GuiPresets::GuiPresets(IAudioEngine& engine, CommandHistory& history,
...
std::string GuiPresets::preset_name_from_path(
...
std::string GuiPresets::preset_path_from_name(
...
void GuiPresets::refresh_presets(bool preserve_selection) {
  ...
  if (selected_preset_index_ >= 0 &&
      selected_preset_index_ < static_cast<int>(preset_files_.size())) {
    std::snprintf(
        preset_name_buf_, sizeof(preset_name_buf_), "%s",
...
bool GuiPresets::save_named_preset(const std::string& preset_name,
  ...
  if (presets_.save_preset(path, preset_name, description, engine_,
                           midi_manager_ ? midi_manager_->mappings()
                                         : std::vector<MidiMapping>())) {
...
void GuiPresets::render_save_popup(bool& show) {
  ...
  ImGui::SetNextItemWidth(-1);
  ImGui::InputTextMultiline("##preset_desc", preset_desc_buf_,
                            sizeof(preset_desc_buf_), ImVec2(-1, 60));

  ImGui::Spacing();
  if (preset_dialog_is_new_) {
    if (ImGui::Button("Save", ImVec2(100, 30))) {
      if (save_named_preset(std::string(preset_name_buf_),
...
void GuiPresets::render_load_popup(bool& show) {
  ...
  if (preset_files_.empty()) {
    ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f),
                       ...
                       "first, or place .json files there.",
