 ...
 */
PedalWidget::PedalWidget(IAudioEngine& engine, std::shared_ptr<Effect> effect,
                         int index)
...
bool PedalWidget::render(float zoom) {
  ...
  float pedal_width = is_mb_comp ? (Theme::PEDAL_WIDTH * 2.2f * zoom)
                                 : (Theme::PEDAL_WIDTH * zoom);
  ...
  // obvious
  if (!enabled && !is_amp) {
    dl->AddRectFilled(p0, p1, Theme::PEDAL_BYPASS_OVERLAY,
  ...
  ImGui::PushStyleVar(ImGuiStyleVar_FramePadding,
                      ImVec2(2.0f * zoom, 2.0f * zoom));

  ImVec4 btn_col = analyzer_open_ ? ImVec4(0.16f, 0.66f, 0.4f, 0.6f)
  ...
  if (is_tuner) {
    ...
    props.on_commit_param_change = [this](int pi, float old_val,
                                          float new_val) {
  ...
  bool is_nam_loader =
      !is_amp && (std::strcmp(effect_->name(), "NAM Loader") == 0);
  if (is_nam_loader) {
  ...
  if (is_looper) {
    ...
    render_knobs(dl, p0, pedal_width, is_amp, is_tuner,
                 is_ir_cab || is_nam_loader, zoom);
  }

  render_footswitch_and_extras(dl, p0, p1, pedal_width, pedal_height, is_amp,
                               enabled, should_remove, zoom);
...
void PedalWidget::render_standard_pedal(ImDrawList* dl, ImVec2 p0, ImVec2 p1,
                                        float pedal_width, bool enabled,
                                        float zoom) {
  ...
  dl->AddRect(p0, p1, Theme::PEDAL_BORDER, Theme::ROUNDING_MD * zoom, 0,
              2.0f * zoom);

  // Metallic top plate
  ImVec2 plate_p0 = ImVec2(p0.x + 8 * zoom, p0.y + 8 * zoom);
  ImVec2 plate_p1 = ImVec2(p1.x - 8 * zoom, p0.y + 45 * zoom);
  dl->AddRectFilled(plate_p0, plate_p1, Theme::PEDAL_PLATE,
...
void PedalWidget::render_footswitch_and_extras(
    ImDrawList* dl, ImVec2 p0, ImVec2 p1, float pedal_width, float pedal_height,
    bool is_amp, bool enabled, bool& should_remove, float zoom) {
  (void)p1;
  (void)should_remove;
  (void)dl;
  bool is_looper = !is_amp && (std::strcmp(effect_->name(), "Looper") == 0);
...
void PedalWidget::commit_param_change(int param_index, float old_val,
                                      float new_val) {
  if (!history_) return;
...
void PedalWidget::render_spectrum_overlay(ImDrawList* dl, ImVec2 pedal_pos,
  ...
  for (float db : ref_lines) {
    ...
    dl->AddLine(ImVec2(overlay_pos.x, y),
                ImVec2(overlay_pos.x + overlay_size.x, y),
  ...
  for (float hz : ticks) {
    ...
    dl->AddLine(ImVec2(x, overlay_pos.y),
                ImVec2(x, overlay_pos.y + overlay_size.y),
  ...
  auto draw_curve =
      [&](const std::array<float, SpectrumAnalyzer::DISPLAY_BARS>& bars,
          ImU32 color) {
        constexpr int BARS = SpectrumAnalyzer::DISPLAY_BARS;
        ImVec2 prev_pt;
        for (int i = 0; i < BARS; ++i) {
          float x = overlay_pos.x +
                    (static_cast<float>(i) / (BARS - 1)) * overlay_size.x;
  ...
  ImGui::SetCursorScreenPos(
      ImVec2(overlay_pos.x + 8.0f * zoom, overlay_pos.y + 6.0f * zoom));
