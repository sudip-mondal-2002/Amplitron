 ...
 */
PedalBoard::PedalBoard(IAudioEngine& engine, CommandHistory& history,
                       GuiMidi* gui_midi)
...
/** @brief Find the index of the current AmpSimulator in the effect chain (-1 if
 * none). */
...
void PedalBoard::render() {
  ...
  ImGui::BeginChild(
      "PedalToolbar", ImVec2(0, bar_height), true,
  ...
  if (ImGui::BeginPopupModal("Confirm Reset##Modal", nullptr,
                             ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::Text(
        "Are you sure you want to reset ALL parameters to their default "
        "values?\nThis will "
        "affect every pedal on the board.");
    ImGui::Separator();
  ...
  if (ImGui::BeginPopupModal("Confirm Clear##Modal", nullptr,
                             ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::Text(
        "Are you sure you want to remove ALL pedals from the signal "
        "chain?\nThis cannot be "
        "undone easily if you have many complex settings.");
    ImGui::Separator();
  ...
  if (ImGui::BeginPopupModal("Confirm MIDI Clear##Modal", nullptr,
                             ImGuiWindowFlags_AlwaysAutoResize)) {
...
/** @brief Add an effect to the chain via undo system, rebuild widgets, and make
 * it visible. */
void PedalBoard::add_effect_and_show(std::shared_ptr<Effect> effect) {
  history_.execute(
      std::make_unique<AddEffectCommand>(engine_, std::move(effect)));
  rebuild_widgets();
}
