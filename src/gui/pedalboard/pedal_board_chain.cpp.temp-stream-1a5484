...
void PedalBoard::render_signal_chain() {
  ...
  for (auto it = ui_state.node_positions.begin();
       it != ui_state.node_positions.end();) {
    bool found = false;
  ...
  ImVec2 offset(canvas_pos.x + ui_state.scrolling.x,
    ...
    if (!target_widget && (node.routing_type == NodeRoutingType::Mixer ||
      ...
      node_height =
          ...
          ui_state.zoom;
               ...
               node.routing_type == NodeRoutingType::Splitter) {
      node_width = 130.0f * ui_state.zoom;
      node_height =
