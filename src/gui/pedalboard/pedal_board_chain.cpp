...
void PedalBoard::render_signal_chain() {
    ...
    ImGuiButtonFlags btn_flags = ImGuiButtonFlags_MouseButtonRight |
    ...
    for (auto it = ui_state.node_positions.begin(); it != ui_state.node_positions.end();) {
        ...
        if (!found) {
            ...
            it = ui_state.node_positions.erase(it);
        } else
            ++it;
    }
    ...
    for (const auto &node : audio_graph.get_nodes()) {
        ...
        if (target_widget &&
            std::strcmp(target_widget->get_effect()->name(), "MultiBand Compressor") == 0) {
            is_mb_comp = true;
        }
        float node_width =
            (target_widget ? (is_mb_comp ? 190.0f * 2.2f : 190.0f) : 110.0f) * ui_state.zoom;
        ...
        if (!target_widget && (node.routing_type == NodeRoutingType::Mixer ||
            ...
            node_height =
                std::max(70.0f, (float)node.input_pin_ids.size() * 32.0f + 55.0f) * ui_state.zoom;
        } else if (!target_widget && node.routing_type == NodeRoutingType::Splitter) {
            node_width = 130.0f * ui_state.zoom;
            node_height =
                std::max(70.0f, (float)node.output_pin_ids.size() * 32.0f + 55.0f) * ui_state.zoom;
        }
        ...
        if (target_widget) {
            ...
            ImGui::SetCursorScreenPos(node_screen_pos);
            ImGui::SetNextItemAllowOverlap();
            ImGui::InvisibleButton("native_drag_handle", ImVec2(node_width - 25.0f * ui_state.zoom,
                                                                30.0f * ui_state.zoom));
            ...
            ImVec2 node_end =
                ImVec2(node_screen_pos.x + node_width, node_screen_pos.y + node_height);
            ...
            draw_list->AddRect(node_screen_pos, node_end, IM_COL32(180, 140, 80, 180),
                               Theme::ROUNDING_MD * ui_state.zoom, 0, 1.5f * ui_state.zoom);
            ImGui::SetCursorScreenPos(node_screen_pos);
            ImGui::SetNextItemAllowOverlap();
            ImGui::InvisibleButton("util_drag_handle",
        ...
        if (is_mixer && ImGui::IsItemHovered() && ImGui::IsMouseClicked(ImGuiMouseButton_Right)) {
            ImGui::OpenPopup("MixerContextMenu");
        }
        if (ImGui::BeginPopup("MixerContextMenu")) {
            if (ImGui::MenuItem("Add Input", nullptr, false, node.input_pin_ids.size() < 8)) {
        ...
        if (is_mixer) {
            ...
            for (size_t idx = 0; idx < node.input_pin_ids.size(); ++idx) {
                float pin_y = node_screen_pos.y +
                              (node_height * (idx + 1.0f) / (node.input_pin_ids.size() + 1.0f));
                ImGui::SetCursorScreenPos(
                    ImVec2(node_screen_pos.x + 8.0f * ui_state.zoom, pin_y - 8.0f * ui_state.zoom));
                ...
                float gain = 1.0f;
                if (idx < node.input_gains.size()) gain = node.input_gains[idx];
        ...
        if (!node.is_reachable) {
            ImVec2 node_end =
                ImVec2(node_screen_pos.x + node_width, node_screen_pos.y + node_height);
            draw_list->AddRectFilled(node_screen_pos, node_end, IM_COL32(0, 0, 0, 180),
                                     Theme::ROUNDING_MD * ui_state.zoom);

            ImVec2 text_pos = ImVec2(node_screen_pos.x + 10.0f * ui_state.zoom,
        ...
        if (node.is_reachable && (!node.input_pin_ids.empty() || !node.output_pin_ids.empty())) {
            ...
            if (!node.output_pin_ids.empty()) {
                out_y = node_screen_pos.y +
                        (node_height * (0 + 1.0f) / (node.output_pin_ids.size() + 1.0f));
            }
            ...
            if (enabled) {
                ...
                draw_list->AddRect(p_min, p_max, IM_COL32(r, g, b, (int)(180 * pulse)),
                                   Theme::ROUNDING_MD * ui_state.zoom, 0, 3.0f * ui_state.zoom);
                draw_list->AddRect(p_min, p_max, IM_COL32(255, 255, 255, (int)(220 * pulse)),
                ...
                auto draw_consistent_glow_line = [&](ImVec2 p1, ImVec2 p2) {
                    ...
                    draw_list->AddLine(p1j, p2j, IM_COL32(r, g, b, (int)(180 * pulse)),
                                       4.0f * ui_state.zoom);
                    draw_list->AddLine(p1j, p2j, IM_COL32(255, 255, 255, (int)(220 * pulse)),
                                       1.5f * ui_state.zoom);
                };
                // Bridge segments
                ...
                for (int e = 0; e < 2; ++e) {
                    ...
                    if (t < 0.2f) {  // Segment 1: Up
                        float st = t / 0.2f;
                        electron_pos =
                            ImVec2(flow_p1.x, flow_p1.y + (rail_y - flow_p1.y) * st + jitter);
                    } else if (t < 0.8f) {  // Segment 2: Across
                        float st = (t - 0.2f) / 0.6f;
                        electron_pos = ImVec2(p_rail_in.x + (p_rail_out.x - p_rail_in.x) * st,
                                              rail_y + jitter);
                    } else {  // Segment 3: Down
                        float st = (t - 0.8f) / 0.2f;
                        electron_pos =
                            ImVec2(p_rail_out.x, rail_y + (flow_p2.y - rail_y) * st + jitter);
                    }

                    draw_list->AddCircleFilled(electron_pos, 3.5f * ui_state.zoom,
        ...
        if (!is_amp && !is_input_node) {
            ImVec2 cross_pos = ImVec2(node_screen_pos.x + node_width - 24.0f * ui_state.zoom,
        ...
        if (!is_input_node) {
            for (size_t idx = 0; idx < node.input_pin_ids.size(); ++idx) {
                ...
                ImGui::SetCursorScreenPos(
                    ImVec2(pin_pos.x - 10.0f * ui_state.zoom, pin_pos.y - 10.0f * ui_state.zoom));
                ImGui::PushID(pin_id);
                ImGui::SetNextItemAllowOverlap();
                ImGui::InvisibleButton("in_pin",
                                       ImVec2(20.0f * ui_state.zoom, 20.0f * ui_state.zoom));
        ...
        if (!is_amp) {
            for (size_t idx = 0; idx < node.output_pin_ids.size(); ++idx) {
                ...
                float pin_y = node_screen_pos.y +
                              (node_height * (idx + 1.0f) / (node.output_pin_ids.size() + 1.0f));
        ...
        if (node_ptr && node_ptr->routing_type == NodeRoutingType::StandardEffect) {
            ...
            for (size_t i = 0; i < effects.size(); ++i) {
                if (effects[i] == node_ptr->pedal) {
                    history_.execute(
                        std::make_unique<RemoveEffectCommand>(engine_, static_cast<int>(i)));
                    rebuild_widgets();
                    break;
                }
            }
    ...
    for (const auto &link : audio_graph.get_links()) {
        if (pin_positions_cache.count(link.source_pin_id) &&
            ...
            for (float t = 0.0f; t <= 1.0f; t += 0.1f) {
                float u = 1.0f - t;
                float px = (u * u * u) * p1.x + (3 * u * u * t) * cp1.x + (3 * u * t * t) * cp2.x +
                           (t * t * t) * p2.x;
                float py = (u * u * u) * p1.y + (3 * u * u * t) * cp1.y + (3 * u * t * t) * cp2.y +
                           (t * t * t) * p2.y;
            ...
            draw_list->AddBezierCubic(p1, cp1, cp2, p2, color,
                                      hovered ? 5.0f * ui_state.zoom : 3.0f * ui_state.zoom);
            ...
            if (is_running) {
                ...
                for (int step = 0; step < 4; ++step) {       // More pulses
                    ...
                    draw_list->AddCircle(ImVec2(hx, hy), pulse_size + 1.0f * ui_state.zoom,
