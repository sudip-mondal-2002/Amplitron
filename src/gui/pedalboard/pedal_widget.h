...
class PedalWidget {
 ...
 private:
  ...
  void render_knob(const char* label, float* value, float min_val,
                   float max_val, float default_val,
                   const char* format = "%.1f");

  /** @brief Render a toggle switch (unused legacy helper). */
  void render_toggle(const char* label, bool* value);

  void render_amp_cabinet(ImDrawList* dl, ImVec2 p0, ImVec2 p1,
                          float pedal_width, float pedal_height, float zoom);
  void render_standard_pedal(ImDrawList* dl, ImVec2 p0, ImVec2 p1,
                             float pedal_width, bool enabled, float zoom);
  void render_nam_loader_display(ImVec2 p0, float pedal_width, float zoom);
  void render_knobs(ImDrawList* dl, ImVec2 p0, float pedal_width, bool is_amp,
                    bool is_tuner, bool is_ir_cab, float zoom);
  void render_footswitch_and_extras(ImDrawList* dl, ImVec2 p0, ImVec2 p1,
