...
class SessionManager {
 ...
 public:
  ...
  bool hasUnsavedSession() const { return fs::exists(autoSavePath); }

  bool shouldSave() const {
    auto now = std::chrono::steady_clock::now();
    return std::chrono::duration_cast<std::chrono::seconds>(now - lastSaveTime)
               .count() >= autoSaveIntervalSeconds;
  }
