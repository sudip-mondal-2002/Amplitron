...
std::vector<float> resample_linear(const std::vector<float>& input,
  ...
  size_t out_len =
      static_cast<size_t>(std::ceil(static_cast<double>(input.size()) / ratio));
  if (out_len == 0) return {};
...
WavData load_wav_file(const std::string& filepath, int target_sample_rate,
                      int max_length_samples) {
  WavData result;

  drwav wav;
  if (!drwav_init_file(&wav, filepath.c_str(), nullptr)) {
    std::cerr << "Cabinet IR: failed to open WAV file: " << filepath
              << std::endl;
    return result;
  }
  ...
  std::vector<float> interleaved(
      static_cast<size_t>(frames_to_read * wav.channels));
  drwav_uint64 frames_read =
      drwav_read_pcm_frames_f32(&wav, frames_to_read, interleaved.data());
  drwav_uninit(&wav);

  if (frames_read == 0) {
    std::cerr << "Cabinet IR: failed to read WAV data: " << filepath
              << std::endl;
  ...
  if (result.channels == 1) {
    ...
    for (size_t f = 0; f < num_frames; ++f) {
      ...
      for (int c = 0; c < ch; ++c)
        sum +=
            interleaved[f * static_cast<size_t>(ch) + static_cast<size_t>(c)];
  ...
  if (target_sample_rate > 0 && target_sample_rate != result.sample_rate) {
    result.samples =
        resample_linear(result.samples, result.sample_rate, target_sample_rate);
  ...
  if (max_length_samples > 0 &&
    ...
    std::cerr << "Cabinet IR: IR truncated from " << result.samples.size()
              << " to " << max_length_samples << " samples" << std::endl;
