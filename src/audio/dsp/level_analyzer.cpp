...
void LevelAnalyzer::update(float input_rms, float output_rms,
                           bool input_clipped, bool output_clipped, float dt) {
  if (is_invalid_dt(dt) || is_invalid_rms(input_rms) ||
      is_invalid_rms(output_rms)) {
    return;
  }
  ...
  input_peak_hold_ =
      std::max(smoothed_input_rms_, input_peak_hold_ - peak_decay * dt);
  output_peak_hold_ =
      std::max(smoothed_output_rms_, output_peak_hold_ - peak_decay * dt);
