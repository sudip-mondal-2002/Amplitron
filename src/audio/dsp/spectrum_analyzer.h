...
/**
 * @brief Performs Fast Fourier Transform (FFT) and frequency magnitude
 * calculations. Completely decoupled from GUI rendering libraries (ImGui/SDL).
 */
class SpectrumAnalyzer {
 public:
   ...
   */
  void update(const float* input_samples, const float* output_samples,
              int sample_rate, float dt_seconds);

  // Getters for display data (so the UI only reads raw arrays)
  const std::array<float, DISPLAY_BARS>& smoothed_input_db() const {
    return smoothed_input_db_;
  }
  const std::array<float, DISPLAY_BARS>& smoothed_output_db() const {
    return smoothed_output_db_;
  }
  const std::array<float, DISPLAY_BARS>& input_peak_db() const {
    return input_peak_db_;
  }
  const std::array<float, DISPLAY_BARS>& output_peak_db() const {
