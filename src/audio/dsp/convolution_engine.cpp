...
ConvolutionKernel::ConvolutionKernel(const std::vector<float>& ir_samples,
  ...
  for (int p = 0; p < num_partitions_; ++p) {
    ...
    for (int i = 0; i < count; ++i) {
      time_buf[static_cast<size_t>(i)].r =
          ir_samples[static_cast<size_t>(offset + i)];
...
void ConvolutionEngine::process_direct(float* buffer, int num_samples) {
  ...
  // We output num_samples and carry over the tail (overlap-add) in
  // direct_overlap_.
  ...
  // Need original input; avoid per-call allocations by using direct_input_
  // scratch.
  if (static_cast<int>(direct_input_.size()) < num_samples) return;
  std::memcpy(direct_input_.data(), buffer,
  ...
  for (int n = 0; n < num_samples; ++n) {
    ...
    for (int j = j0; j <= j1; ++j) {
      y += direct_input_[static_cast<size_t>(n - j)] *
           ir[static_cast<size_t>(j)];
    }
    buffer[n] = y;
  }
  ...
  for (int t = 0; t < tail_len; ++t) {
    ...
    // y += sum_{j=0}^{ir_len-1} x[n-j] * h[j], where (n-j) in [0,
    // num_samples-1]
    int j0 = std::max(0, n - (num_samples - 1));
    int j1 = std::min(ir_len - 1, n);
    for (int j = j0; j <= j1; ++j) {
      ...
      if (xi >= 0 && xi < num_samples) {
        y +=
            direct_input_[static_cast<size_t>(xi)] * ir[static_cast<size_t>(j)];
      }
    }
...
void ConvolutionEngine::process(float* buffer, int num_samples) {
  ...
  // Also use direct convolution for very short IRs (1 partition, IR <=
  // block_size)
  ...
  for (int k = 0; k < num_parts; ++k) {
    ...
    const auto* ir_block =
        reinterpret_cast<const kiss_fft_cpx*>(kernel_->partition_freq(k));
