...
SpectrumAnalyzer::SpectrumAnalyzer() {
  for (int i = 0; i < FFT_SIZE; ++i) {
    const float phase =
        (TWO_PI * static_cast<float>(i)) / static_cast<float>(FFT_SIZE - 1);
...
void SpectrumAnalyzer::run_fft(
    std::array<std::complex<float>, FFT_SIZE>& data) const {
...
void SpectrumAnalyzer::compute_spectrum_bars(
    const float* samples, int sample_rate,
  ...
  for (int i = 1; i < FFT_BINS; ++i) {
    ...
    mags_db[i] =
        clamp(20.0f * std::log10(std::max(mag, 1e-8f)), kMinDb, kMaxDb);
  }

  for (int bar = 0; bar < DISPLAY_BARS; ++bar) {
    const float t0 = static_cast<float>(bar) / static_cast<float>(DISPLAY_BARS);
    const float t1 =
        static_cast<float>(bar + 1) / static_cast<float>(DISPLAY_BARS);
    const float hz0 =
        std::pow(10.0f, lerp(std::log10(kMinHz), std::log10(kMaxHz), t0));
    const float hz1 =
        std::pow(10.0f, lerp(std::log10(kMinHz), std::log10(kMaxHz), t1));

    int bin0 =
        static_cast<int>(hz0 * FFT_SIZE / static_cast<float>(sample_rate));
    int bin1 =
        static_cast<int>(hz1 * FFT_SIZE / static_cast<float>(sample_rate));
...
void SpectrumAnalyzer::update(const float* input_samples,
                              const float* output_samples, int sample_rate,
                              float dt_seconds) {
  ...
  const float decay =
      peak_decay_db_per_sec * std::max(dt_seconds, 1.0f / 240.0f);

  for (int i = 0; i < DISPLAY_BARS; ++i) {
    smoothed_input_db_[i] =
        lerp(smoothed_input_db_[i], input_db[i], smooth_alpha);
    smoothed_output_db_[i] =
        lerp(smoothed_output_db_[i], output_db[i], smooth_alpha);

    input_peak_db_[i] =
        std::max(smoothed_input_db_[i], input_peak_db_[i] - decay);
    output_peak_db_[i] =
        std::max(smoothed_output_db_[i], output_peak_db_[i] - decay);
  }
}
