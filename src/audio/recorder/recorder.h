...
class Recorder : public IRecorder {
 public:
  ...
  bool start(const std::string& filepath, int sample_rate,
  ...
  void write_samples_stereo(const float* left, const float* right,
                            int num_samples) override;

  // Write metadata JSON sidecar file
  void write_metadata(const std::string& wav_path,
                      IAudioEngine& engine) override;
  ...
  int64_t get_samples_written() const override {
    return samples_written_.load();
  }
