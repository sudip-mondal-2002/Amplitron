...
class IRecorder {
 public:
  ...
  virtual bool start(const std::string& filepath, int sample_rate,
                     int channels = 1) = 0;
  ...
  virtual void write_samples_stereo(const float* left, const float* right,
                                    int num_samples) = 0;

  // Write metadata JSON sidecar file
  virtual void write_metadata(const std::string& wav_path,
                              IAudioEngine& engine) = 0;
