...
void Recorder::writer_thread_func() {
    ...
    if (available > 0) {
      ...
      while (available > 0) {
        int chunk = static_cast<int>(
            std::min(available, static_cast<int64_t>(pcm_buffer_.size())));
  ...
  while (remaining > 0) {
    int chunk = static_cast<int>(
        std::min(remaining, static_cast<int64_t>(pcm_buffer_.size())));
    for (int i = 0; i < chunk; ++i) {
      float s = ring_buffer_[static_cast<int>((rp + i) % RING_BUFFER_SIZE)];
      if (s > 1.0f) s = 1.0f;
      if (s < -1.0f) s = -1.0f;
...
void Recorder::write_samples_stereo(const float* left, const float* right,
                                    int num_samples) {
  if (!recording_ || paused_) return;
