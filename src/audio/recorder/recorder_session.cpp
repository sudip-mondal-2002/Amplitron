...
bool Recorder::start(const std::string& filepath, int sample_rate,
                     int channels) {
  ...
  // Start the disk writer thread (keeps file I/O off the real-time audio
  // thread)
...
bool Recorder::save_to(const std::string& dest_path) {
  ...
  if (!dst.good() || !src.good()) {
    std::cerr << "Recorder: failed to copy recording to " << dest_path
              << std::endl;
    src.close();
    dst.close();
    return false;
