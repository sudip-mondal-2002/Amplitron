...
TEST(command_history_execute_enables_undo) {
  ...
  CommandHistory ch;
  ch.execute(std::make_unique<AddEffectCommand>(engine,
...
static bool is_version_newer(const std::string& latest,
...
TEST(update_extract_html_url_found) {
  ...
  ASSERT_EQ(extract_html_url(json),
            std::string("https://github.com/releases/v1.0.0"));
}
