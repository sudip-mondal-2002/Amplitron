...
bool JackBackend::initialize(IAudioEngine *engine) {
  ...
  if (!client_) {
    ...
    std::cerr << "[Amplitron] JACK: could not open JACK server (is jackd "
                 "running?). Status: 0x"
...
bool JackBackend::start() {
  ...
  if (!ports_registered_) {
    in_port_ = jack_port_register(client_, "in_1", JACK_DEFAULT_AUDIO_TYPE,
                                  JackPortIsInput, 0);
    out_port_ = jack_port_register(client_, "out_1", JACK_DEFAULT_AUDIO_TYPE,
                                   JackPortIsOutput, 0);
    if (!in_port_ || !out_port_) {
      std::cerr << "[Amplitron] JACK: failed to register in/out ports."
                << std::endl;
      return false;
    }
  ...
  if (jack_activate(client_)) {
    std::cerr << "[Amplitron] JACK: failed to activate JACK client."
              << std::endl;
    return false;
  }
...
std::vector<AudioDeviceInfo> JackBackend::get_input_devices() const {
  return {
      {0, "JACK in_1", 1, 0, static_cast<double>(get_sample_rate()), false}};
}

std::vector<AudioDeviceInfo> JackBackend::get_output_devices() const {
  return {
      {0, "JACK out_1", 0, 1, static_cast<double>(get_sample_rate()), false}};
}
