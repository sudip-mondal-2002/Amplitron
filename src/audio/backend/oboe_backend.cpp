 ...
 public:
  ...
  oboe::DataCallbackResult onAudioReady(oboe::AudioStream* /*stream*/,
    ...
    if (available >= numFrames) {
      ...
      std::memcpy(capture_buffer_.data(), capture_ring_.data() + read_pos,
                  static_cast<size_t>(firstChunk) * sizeof(float));
      if (firstChunk < numFrames) {
        int secondChunk = numFrames - firstChunk;
        std::memcpy(capture_buffer_.data() + firstChunk, capture_ring_.data(),
                    static_cast<size_t>(secondChunk) * sizeof(float));
      }
      capture_read_pos_.store((read_pos + numFrames) % kRingSize,
  ...
  void feedCaptureData(const float* data, int numFrames) {
    ...
    std::memcpy(capture_ring_.data() + write_pos, data,
                static_cast<size_t>(firstChunk) * sizeof(float));
    if (firstChunk < toCopy) {
      int secondChunk = toCopy - firstChunk;
      std::memcpy(capture_ring_.data(), data + firstChunk,
                  static_cast<size_t>(secondChunk) * sizeof(float));
    }
    capture_write_pos_.store((write_pos + toCopy) % kRingSize,
                             std::memory_order_release);
    capture_filled_.fetch_add(toCopy, std::memory_order_release);
  }

  oboe::SharingMode get_sharing_mode() const {
    return negotiated_sharing_mode_;
  }
...
OboeBackend::OboeBackend() { impl_ = new OboeBackendImpl(); }
...
bool OboeBackend::start() {
  ...
  impl->captureCallback =
      std::make_unique<OboeCaptureCallback>(impl->playbackCallback.get());
...
std::vector<AudioDeviceInfo> OboeBackend::get_input_devices() const {
  ...
  devices.push_back({0, "Default (Auto-select)", 1, 0,
                     static_cast<double>(sample_rate_), false});
  if (impl->usb_input_device_id >= 0) {
    devices.push_back({impl->usb_input_device_id, "USB Guitar Cable", 1, 0,
                       static_cast<double>(sample_rate_), true});
  }
  return devices;
}
...
std::vector<AudioDeviceInfo> OboeBackend::get_output_devices() const {
  auto* impl = static_cast<OboeBackendImpl*>(impl_);
  std::vector<AudioDeviceInfo> devices;
  devices.push_back({0, "Default (Auto-select)", 0, 2,
                     static_cast<double>(sample_rate_), false});
  if (impl->usb_output_device_id >= 0) {
    devices.push_back({impl->usb_output_device_id, "USB Guitar Cable (output)",
                       0, 2, static_cast<double>(sample_rate_), true});
  }
  return devices;
}
...
bool OboeBackend::set_input_device(int device_index) {
  ...
  impl->input_device_name =
      (device_index > 0) ? "USB Guitar Cable" : "Android Microphone";
  return true;
}
...
bool OboeBackend::set_output_device(int device_index) {
  ...
  impl->output_device_name =
      (device_index > 0) ? "USB Guitar Cable (output)" : "Android Speaker";
  return true;
}
...
const char* OboeBackend::get_oboe_sharing_mode_label() const {
  ...
  return (impl->playbackCallback->get_sharing_mode() ==
