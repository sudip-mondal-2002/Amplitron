...
bool SdlBackend::start() {
  ...
  if (audio_device_ == 0) {
    std::cerr << "[SDL] Failed to open output audio: " << SDL_GetError()
              << std::endl;
    return false;
  }
  ...
  capture_device_ =
      SDL_OpenAudioDevice(preferred_device, 1, &want_cap, &have_cap,
                          SDL_AUDIO_ALLOW_FREQUENCY_CHANGE);
...
std::vector<AudioDeviceInfo> SdlBackend::get_input_devices() const {
  std::vector<AudioDeviceInfo> dev = {
      {0, "Browser Microphone", 1, 0, 48000.0, false}};
  return dev;
}

std::vector<AudioDeviceInfo> SdlBackend::get_output_devices() const {
  std::vector<AudioDeviceInfo> dev = {
      {0, "Browser Audio Output", 0, 2, 48000.0, false}};
  return dev;
}
