...
bool PortAudioBackend::start() {
  ...
  PaError err = Pa_OpenStream(&stream_, &input_params, &output_params,
                              sample_rate, frames, paClipOff | paDitherOff,
                              pa_audio_callback, engine_);

  if (err != paNoError) {
    std::cerr << "Failed to open PortAudio stream: " << Pa_GetErrorText(err)
              << std::endl;

    // Adjust parameters before retrying: disable WASAPI exclusive and reset
    // latency suggestions
    input_params.hostApiSpecificStreamInfo = nullptr;
    output_params.hostApiSpecificStreamInfo = nullptr;
    ...
    // Retry
    err = Pa_OpenStream(&stream_, &input_params, &output_params, sample_rate,
                        buffer_size, paClipOff | paDitherOff, pa_audio_callback,
                        engine_);
    if (err != paNoError) {
      std::cerr << "PortAudio open stream retry failed: "
