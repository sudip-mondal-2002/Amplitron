...
AudioEngine::AudioEngine(std::unique_ptr<IRecorder> recorder,
  ...
  // Assuming standard values, use your engine's actual sample rate / block size
  // variables here
  main_executor_->prepare(48000, 512, 32);

  // Seed the shadow executor so the audio thread has something safe to read
  // instantly
...
void AudioEngine::set_buffer_size(int size) {
  ...
  if (was_running) {
    if (!start()) {
      last_error_ =
          "Failed with buffer size " + std::to_string(size) + ". Reverting.";
...
void AudioEngine::set_sample_rate(int rate) {
...

  {
    ...
    for (const auto& node : main_graph_.get_nodes()) {
      if (node.pedal) {  // Check if it's a standard effect and not a bare merge
                         // node
  ...
  if (was_running) {
    if (!start()) {
      last_error_ =
          "Failed with sample rate " + std::to_string(rate) + " Hz. Reverting.";
      std::cerr << "[Amplitron] " << last_error_ << std::endl;
    ...
    if (was_running) {
      if (!start()) {
        last_error_ = "Failed with sample rate " + std::to_string(rate) +
                      " Hz. Reverting.";
        std::cerr << "[Amplitron] " << last_error_ << std::endl;
        sample_rate_ = prev_rate;
...
void AudioEngine::commit_graph_changes() {
  ...
  // 1. Create a brand new executor (so we don't mutate memory the audio thread
  ...
  // 3. Promote it to the main slot. The audio thread will grab it on the next
  // try_lock!
...
class NonOwningBackendWrapper : public IAudioBackend {
 public:
  explicit NonOwningBackendWrapper(IAudioBackend* delegate)
      : delegate_(delegate) {}

  bool initialize(IAudioEngine* engine) override {
    return delegate_->initialize(engine);
  }
  ...
  std::vector<AudioDeviceInfo> get_input_devices() const override {
    return delegate_->get_input_devices();
  }
  std::vector<AudioDeviceInfo> get_output_devices() const override {
    return delegate_->get_output_devices();
  }

  bool set_input_device(int index) override {
    return delegate_->set_input_device(index);
  }
  bool set_output_device(int index) override {
    return delegate_->set_output_device(index);
  }

  std::string get_input_device_name() const override {
    return delegate_->get_input_device_name();
  }
  std::string get_output_device_name() const override {
    return delegate_->get_output_device_name();
  }
  ...
  int get_input_device() const override {
    return delegate_->get_input_device();
  }
  int get_output_device() const override {
    return delegate_->get_output_device();
  }
...
void AudioEngine::replace_backend_state_for_test(
...
void AudioEngine::clear_backend_for_test() { backend_ = nullptr; }
#endif
...
bool AudioEngine::start() {
  ...
  if (backend_) {
    ...
    if (running_) {
      ...
      // Pre-allocate the audio thread buffers to avoid allocations in the
      // realtime callback
      ...
      // Recompile graph executor with actual sample_rate and buffer_size from
      // audio hardware
...
bool AudioEngine::set_input_device(int device_index) {
  ...
  if (was_running) {
    if (!start()) {
      ...
      if (!start()) {
        last_error_ =
            "Failed to revert to previous input device. Engine stopped.";
        std::cerr << "[Amplitron] " << last_error_ << std::endl;
      }
      return false;
    }
...
bool AudioEngine::set_output_device(int device_index) {
  ...
  if (was_running) {
    if (!start()) {
      ...
      if (!start()) {
        last_error_ =
            "Failed to revert to previous output device. Engine stopped.";
        std::cerr << "[Amplitron] " << last_error_ << std::endl;
      }
      return false;
    }
