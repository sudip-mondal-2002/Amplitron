...
void AudioEngine::sync_graph_with_dummy_effects(bool reset_graph) {
  {
    ...
    if (main_graph_.get_nodes().empty()) {
      ...
      int input_node_id = main_graph_.add_node(
          "Input", NodeRoutingType::StandardEffect, nullptr);
      ...
      int prev_output_pin =
          main_graph_.get_nodes().back().output_pin_ids.empty()
              ? -1
      ...
      for (auto& fx : dummy_effects_) {
        ...
        fx->reset();
        int node_id = main_graph_.add_node(fx->name(),
                                           NodeRoutingType::StandardEffect, fx);
      ...
      // Mark the last node in the linear chain as the Output so sound reaches
      // the speakers
      if (!main_graph_.get_nodes().empty()) {
        main_graph_.set_node_as_output(main_graph_.get_nodes().back().id, true);
      }
    } else {
      // 2. MODULAR MODE: Remove standard nodes that are no longer in
      // dummy_effects_
      std::vector<int> nodes_to_remove;
      for (const auto& node : main_graph_.get_nodes()) {
        if (node.routing_type == NodeRoutingType::StandardEffect &&
            node.pedal != nullptr) {
          auto it = std::find(dummy_effects_.begin(), dummy_effects_.end(),
                              node.pedal);
      ...
      // Add standard nodes for effects in dummy_effects_ that are not yet in
      // the graph
      for (int i = 0; i < static_cast<int>(dummy_effects_.size()); ++i) {
        ...
        if (!exists) {
          fx->set_sample_rate(sample_rate_);
          fx->reset();
          int node_id = main_graph_.add_node(
              fx->name(), NodeRoutingType::StandardEffect, fx);
...
void AudioEngine::restore_effects_state(
    std::vector<std::shared_ptr<Effect>> state) {
