  ...
  for (auto &node : nodes_) {
    if (node.id == node_id && node.routing_type == NodeRoutingType::Mixer) {
      if (node.input_pin_ids.size() < 8) {
        ...
        // Do not necessarily need to rebuild topology if we just added an
        // unconnected pin
        return true;
      }
      return false;
...
bool AudioGraph::remove_input_pin(int node_id, int pin_id) {
  for (auto &node : nodes_) {
    if (node.id == node_id &&
        (node.routing_type == NodeRoutingType::Mixer ||
      ...
      if (node.input_pin_ids.size() > 2) {
        ...
        if (index_to_remove != -1) {
          ...
          node.input_pin_ids.erase(node.input_pin_ids.begin() +
                                   index_to_remove);
...
void AudioGraph::restore_input_pin(int node_id, int pin_id, int index,
                                   float gain) {
  for (auto &node : nodes_) {
    if (node.id == node_id) {
      if (index >= 0) {
...
void AudioGraph::set_mixer_input_gain(int node_id, size_t pin_index,
                                      float gain) {
  for (auto &node : nodes_) {
    if (node.id == node_id && node.routing_type == NodeRoutingType::Mixer) {
    ...
    if (node.id == node_id && node.routing_type == NodeRoutingType::Splitter) {
      if (node.output_pin_ids.size() > 2) {
        ...
        if (index_to_remove != -1) {
          ...
          node.output_pin_ids.erase(node.output_pin_ids.begin() +
                                    index_to_remove);
          return true;
        }
      }
...
void AudioGraph::restore_output_pin(int node_id, int pin_id, int index) {
  for (auto &node : nodes_) {
    if (node.id == node_id && node.routing_type == NodeRoutingType::Splitter) {
      if (index >= 0 &&
          static_cast<size_t>(index) <= node.output_pin_ids.size()) {
