...
TempoEngine::TempoEngine() {
  ...
  for (int i = 0; i < 512; ++i) {
    hann_window_[i] =
        static_cast<float>(0.5 * (1.0 - std::cos(2.0 * M_PI * i / 511.0)));
  }
...
float TempoEngine::detect_bpm() {
  ...
  // Need at least 2 seconds of audio to detect BPM reliably (especially down to
  // 60 BPM)
  ...
  if (written < buffer_.size()) {
    std::copy(buffer_.begin(), buffer_.begin() + size_to_copy,
              local_audio_.begin());
  } else {
    size_t pos = write_pos_;
    std::copy(buffer_.begin() + pos, buffer_.end(), local_audio_.begin());
    std::copy(buffer_.begin(), buffer_.begin() + pos,
              local_audio_.begin() + (buffer_.size() - pos));
  }
  ...
  int num_frames =
      (static_cast<int>(local_audio_.size()) - frame_size) / hop_size;
  if (num_frames <= 2) {
    return -1.0f;
  }
  ...
  for (int f = 0; f < num_frames; ++f) {
    ...
    for (int i = 0; i <= frame_size / 2; ++i) {
      mag[i] = std::sqrt(freq_out[i].r * freq_out[i].r +
                         freq_out[i].i * freq_out[i].i);
    }
  ...
  for (int factor : check_factors) {
    int candidate_lag =
        static_cast<int>(std::round(static_cast<float>(best_lag) / factor));
