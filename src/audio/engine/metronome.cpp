...
void Metronome::toggle() {
  ...
  while (!enabled_.compare_exchange_weak(current, !current,
...
void Metronome::update_timing() {
  ...
  const int click_samples =
      std::max(1, static_cast<int>(sample_rate * kClickLengthSec + 0.5f));
  metronome_click_samples_total_ = click_samples;

  constexpr float kClickFreq = 1000.0f;
  metronome_click_phase_inc_ =
      (kTwoPi * kClickFreq) / static_cast<float>(sample_rate);

  const float target = 0.001f;
  metronome_click_decay_ =
      std::exp(std::log(target) / static_cast<float>(click_samples));
}
...
float Metronome::next_sample() {
  ...
  metronome_volume_smoothed_ +=
      metronome_volume_smooth_alpha_ *
      (metronome_volume_ - metronome_volume_smoothed_);
  ...
  float click = std::sin(metronome_click_phase_) * metronome_click_env_ *
                metronome_volume_smoothed_;
