 ...
 * - Precedence: poly_backend_ (if non-null) takes operational precedence over
 * the native backend_.
 * - Ownership: AudioEngine owns the opaque backend_ pointer (manages its
 * creation/destruction). However, AudioEngine does NOT take ownership of the
 * polymorphic poly_backend_ pointer; the caller or test fixture is responsible
...
class AudioEngine : public IAudioEngine {
 public:
  ...
  /** @brief Stop and restart the stream (manual recovery). @return true on
   * success. */
  ...
  /**
   * @brief Return a human-readable label for the Oboe sharing mode negotiated
   * at runtime. "AAudio exclusive mode" when AAudio exclusive path is active;
   * "OpenSL ES (shared)" otherwise. Used by the Android settings UI to display
   * the actual backend, not a hardcoded string.
   */
  ...
  /** @brief Return the current input device index (delegates to backend so
   * auto-detection is reflected). */
  int get_input_device() const override {
    return backend_ ? backend_->get_input_device() : -1;
  }

  /** @brief Return the current output device index (delegates to backend so
   * auto-detection is reflected). */
  int get_output_device() const override {
    return backend_ ? backend_->get_output_device() : -1;
  }
  ...
  std::vector<float>& test_process_buffer_right() {
    return process_buffer_right_;
  }
  ...
  std::vector<std::shared_ptr<Effect>>& effects() override {
    return dummy_effects_;
  }

  void add_effect(std::shared_ptr<Effect> fx) override;
  void add_initial_effects(
      const std::vector<std::shared_ptr<Effect>>& fxs) override {
  ...
  void restore_effects_state(
      std::vector<std::shared_ptr<Effect>> state) override;
  ...
  float get_input_rms() const override {
    return input_rms_.load(std::memory_order_relaxed);
  }

  /** @brief Return the most recent output RMS level (0.0–1.0, atomic). */
  float get_output_rms() const override {
  ...
  /** @brief Snapshot sequence counter; increments when new analyzer data is
   * published. */
   ...
   * @param sample_count Number of samples to copy (clamped to
   * ANALYZER_FFT_SIZE).
  ...
  /**
   * @brief Set the master output gain (enqueued to audio thread via SPSC
   * queue).
   * @param gain Linear gain multiplier.
   */
  ...
  float get_output_gain() const override {
    return output_gain_.load(std::memory_order_relaxed);
  }
   ...
   */
  bool get_metronome_enabled() const override {
    return metronome_->is_enabled();
  }
  ...
  float get_metronome_volume() const override {
    return metronome_->get_volume();
  }
   ...
   */
  void push_param_change(int effect_index, int param_index,
  ...
  void set_auto_buffer_enabled(bool enabled) override {
    auto_buffer_enabled_ = enabled;
  }
