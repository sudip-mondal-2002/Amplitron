...
void AudioEngine::process_audio(const float* input, float* output,
                                int frame_count) {
  ...
  command_dispatcher_.drain_gain_commands(input_gain_, output_gain_,
                                          audio_shadow_executor_);

  if (effect_mutex_.try_lock()) {
    command_dispatcher_.drain_commands(input_gain_, output_gain_,
  ...
  if (audio_shadow_executor_) {
    // Broadcast tempo/bpm
    audio_shadow_executor_->update_transport_state(
        global_bpm_.load(std::memory_order_relaxed));

    // Pass your mono/stereo buffers to the executor we built
    audio_shadow_executor_->process(process_buffer_.data(),
  ...
  if (analyzer_on) {
    ...
    for (int i = 0; i < frame_count; ++i) {
      ...
      float out_r =
          clamp(process_buffer_right_[i] * out_gain + click, -1.0f, 1.0f);
      if (std::fabs(out_l) >= 1.0f || std::fabs(out_r) >= 1.0f)
        clipped_out = true;
      output[i * 2] = out_l;
      output[i * 2 + 1] = out_r;
  ...
  float duration_us =
      std::chrono::duration<float, std::micro>(t_end - t_start).count();
