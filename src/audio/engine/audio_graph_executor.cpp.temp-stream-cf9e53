...
void AudioGraphExecutor::prepare(int sample_rate, int max_block_size,
                                 int max_nodes) {
  sample_rate_ = sample_rate;
  max_block_size_ = max_block_size;
  max_nodes_ = max_nodes;

  // Pre-allocate the memory pool to guarantee no allocations on the audio
  // thread
      ...
      for (const auto& link : links) {
        if (link.dest_pin_id == in_pin) {
          ...
          if (src_node_id != -1 && node_to_buffer.count(src_node_id)) {
            step.input_sources.push_back({node_to_buffer[src_node_id], pin_gain,
                                          static_cast<int>(pin_idx)});
          }
        }
      }
    }
...
void AudioGraphExecutor::process(const float* input, float* output,
                                 int num_samples) {
  ...
  // Accumulate/mix outputs from all explicit sink nodes into the final output
  // buffer
  std::memset(output, 0, num_samples * sizeof(float));
  for (const auto& step : execution_plan_) {
    if (step.is_sink) {
...
void AudioGraphExecutor::update_mixer_gain(int node_id, int pin_index,
                                           float gain) {
  for (auto& step : execution_plan_) {
    if (step.node_id == node_id && (step.type == NodeRoutingType::Mixer ||
                                    step.type == NodeRoutingType::MergeSum)) {
