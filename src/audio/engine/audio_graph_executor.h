...
class StandardEffectProcessor : public INodeProcessor {
 public:
  explicit StandardEffectProcessor(std::shared_ptr<Effect> pedal)
      : pedal_(std::move(pedal)) {}
...
class AudioGraphExecutor {
 public:
  ...
  struct NodeExecutionStep {
    ...
    std::vector<InputSource>
        input_sources;  // Which buffers to sum together for the input
  ...
  // Allocate the memory pool ahead of time (Call this during engine
  // initialization)
  ...
  // Adjust the pedal->process signature if your pedals process strictly
  // in-place
  ...
  /** @brief Direct read-only access to the execution plan for white-box
   * testing. */
  const std::vector<NodeExecutionStep>& test_execution_plan() const {
 ...
 private:
  ...
  // Pre-allocated memory for routing parallel signal streams:
  // [node_capacity][max_block_size]
