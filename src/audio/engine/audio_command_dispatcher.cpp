...
void AudioCommandDispatcher::push_param_change(int effect_index,
...
void AudioCommandDispatcher::push_mixer_gain_change(int node_id, int pin_index,
                                                    float gain) {
  AudioCommand cmd{};
...
void AudioCommandDispatcher::push_effect_enabled(int effect_index,
                                                 float enabled) {
  AudioCommand cmd{};
...
void AudioCommandDispatcher::drain_gain_commands(
    std::atomic<float>& input_gain, std::atomic<float>& output_gain,
  ...
  while (command_queue_.try_peek(cmd)) {
    if (cmd.type == AudioCommand::SetInputGain) {
      ...
      command_queue_.try_pop(cmd);
      if (executor) {
        executor->update_mixer_gain(cmd.effect_index, cmd.param_index,
                                    cmd.value);
      }
    } else {
      break;
...
void AudioCommandDispatcher::drain_commands(
    std::atomic<float>& input_gain, std::atomic<float>& output_gain,
  ...
  while (command_queue_.try_pop(cmd)) {
    // Helper to find the effect pointer safely in the compiled executor by
    // node_id, with fallback
    auto get_effect_by_id = [&](int node_id) -> std::shared_ptr<Effect> {
      ...
      if (node_id >= 0 && node_id < static_cast<int>(dummy_effects.size())) {
        // Comments on node_id semantics: node_id is used as a 0-based linear
        // index fallback for the GUI and tests
        std::cerr << "[AudioCommandDispatcher] Node ID " << node_id
                  << " not found in executor or graph; falling back to "
                     "dummy_effects index."
                  << std::endl;
