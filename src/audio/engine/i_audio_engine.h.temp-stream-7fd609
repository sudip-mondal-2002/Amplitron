...
/**
 * @brief Abstract interface for audio device, latency, and sample rate
 * management. Satisfies the Interface Segregation Principle (ISP) and
...
/**
 * @brief Abstract interface for audio level, RMS, clipping, and CPU load
 * metrics. Satisfies the Interface Segregation Principle (ISP) and Dependency
 * Inversion Principle (DIP).
 * @note Threading Contract: Safe to call from the GUI/Main thread. Reads atomic
 * variables updated by the audio thread.
 */
 ...
 * @note Threading Contract: Call only from the GUI/Main thread. Modifies graph
 * structure.
 */
class IEffectChain {
 public:
  ...
  virtual void add_initial_effects(
      const std::vector<std::shared_ptr<Effect>>& fxs) = 0;
  ...
  virtual void restore_effects_state(
      std::vector<std::shared_ptr<Effect>> state) = 0;
};

/**
 * @brief Interface for transport settings, master gains, and metronome
 * controls.
 * @note Threading Contract: Call only from the GUI/Main thread.
 */
 ...
 * @note Threading Contract: Called exclusively from the high-priority real-time
 * audio callback thread. Must be lock-free and memory-allocation-free.
 */
class IAudioProcessor {
 public:
  virtual ~IAudioProcessor() = default;
  virtual void process_audio(const float* input, float* output,
                             int frame_count) = 0;
};

/**
 * @brief Interface for configuring and retrieving FFT analyzer snapshots.
 * @note Threading Contract: Safe to call from the GUI/Main thread. Uses locks
 * to copy shared buffers.
 */
 ...
 * @note Threading Contract: Safe to call from any thread (non-blocking
 * lock-free SPSC queue).
 */
class IParameterDispatch {
 public:
  virtual ~IParameterDispatch() = default;
  virtual void push_param_change(int effect_index, int param_index,
                                 float value) = 0;
  virtual void push_mixer_gain_change(int node_id, int pin_index,
                                      float gain) = 0;
...
/**
 * @brief Interface for accessing and committing topological audio graph
 * changes.
 * @note Threading Contract: Call only from the GUI/Main thread.
 */
