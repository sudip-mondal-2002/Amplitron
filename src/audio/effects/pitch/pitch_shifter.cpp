...
void PitchShifter::build_hann_lut() {
  for (size_t i = 0; i < hann_lut_.size(); ++i) {
    ...
    hann_lut_[i] =
        0.5f * (1.0f - std::cos(TWO_PI * static_cast<float>(i) / 8192.0f));
  }
}
...
PitchShifter::PitchShifter() {
  params_ = {
      {"Shift", 0.0f, -12.0f, 12.0f, 0.0f, "st",
       "Pitch shift in semitones. Negative shifts down, positive shifts up. 12 "
       "= one octave."},
      {"Fine", 0.0f, -50.0f, 50.0f, 0.0f, "ct",
       "Fine-tune adjustment in cents (hundredths of a semitone) for precise "
       "detuning."},
...
void PitchShifter::process(float* buffer, int num_samples) {
  ...
  const float block_alpha = 1.0f - std::exp(-static_cast<float>(num_samples) /
                                            (sample_rate_ * 0.010f));
