 ...
 * to handle bypass (enabled/disabled) and wet/dry mix computation in a clean,
 * unified way.
 *
 * Satisfies the Single Responsibility Principle (SRP) by offloading
 * bypass/mixing math from individual DSP algorithms.
 */
class EffectNode : public IProcessor {
 public:
  ...
  void process(float* buffer, int num_samples) override {
    ...
    std::memcpy(dry_buffer_.data(), buffer,
    ...
    for (int i = 0; i < num_samples; ++i) {
      buffer[i] =
          dry_buffer_[i] * (1.0f - current_mix) + buffer[i] * current_mix;
    }
  }
  ...
  void process_stereo(float* left, float* right, int num_samples) override {
    ...
    std::memcpy(dry_l_buffer_.data(), left,
                static_cast<size_t>(num_samples) * sizeof(float));
    std::memcpy(dry_r_buffer_.data(), right,
                static_cast<size_t>(num_samples) * sizeof(float));
    ...
    for (int i = 0; i < num_samples; ++i) {
      ...
      right[i] =
          dry_r_buffer_[i] * (1.0f - current_mix) + right[i] * current_mix;
    }
  }
