...
class Effect : public IProcessor,
 ...
 public:
  ...
  virtual void process_stereo(float* left, float* right,
                              int num_samples) override {
    process(left, num_samples);
    std::memcpy(right, left, static_cast<size_t>(num_samples) * sizeof(float));
  }

  // Update the processing sample rate before audio starts or after device
  // changes.
  ...
  void set_mix(float mix) {
    mix_.store(clamp(mix, 0.0f, 1.0f), std::memory_order_relaxed);
  }
