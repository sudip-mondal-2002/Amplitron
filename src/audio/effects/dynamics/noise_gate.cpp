...
NoiseGate::NoiseGate() {
  params_ = {
      {"Threshold", -55.0f, -80.0f, 0.0f, -55.0f, "dB",
       "Signal level below which the gate closes and mutes the audio. Set just "
       "above background "
       "noise level."},
      {"Attack", 0.5f, 0.1f, 10.0f, 0.5f, "ms",
       "How quickly the gate opens when the signal exceeds the threshold. Fast "
       "attack preserves "
       "pick transients."},
      {"Release", 50.0f, 5.0f, 500.0f, 50.0f, "ms",
       "How quickly the gate closes after the signal falls below threshold. "
       "Longer release "
...
void NoiseGate::process(float* buffer, int num_samples) {
  ...
  float attack_coeff =
      EnvelopeFollower::time_to_coeff(params_[1].value, sample_rate_);
  float release_coeff =
      EnvelopeFollower::time_to_coeff(params_[2].value, sample_rate_);
