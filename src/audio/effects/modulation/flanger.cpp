...
Flanger::Flanger() {
  params_ = {
      {"Rate", 0.3f, 0.05f, 5.0f, 0.3f, "Hz",
       "LFO speed. Slow rates create a dreamy sweep; fast rates push toward "
       "vibrato territory."},
      {"Depth", 2.0f, 0.1f, 7.0f, 2.0f, "ms",
       "Modulation depth in milliseconds. Controls the width of the delay time "
       "sweep around the "
       "base delay."},
      {"Delay", 1.0f, 0.1f, 10.0f, 1.0f, "ms",
       "Base delay time. Short values (< 2ms) create tight comb filtering; "
       "longer values soften "
       "the effect."},
      {"Feedback", 0.7f, -0.95f, 0.95f, 0.7f, "",
       "Feedback amount. Positive values add metallic resonance; negative "
       "values create a "
       "hollow, phasey character."},
      {"Mix", 0.5f, 0.0f, 1.0f, 0.5f, "",
       "Dry/wet blend. At 0.5 the comb filter notches are deepest for the "
       "classic flanger "
       "sound."},
  };
...
void Flanger::set_sample_rate(int sample_rate) {
  ...
  max_delay_samples_ =
      static_cast<int>(sample_rate * MAX_DELAY_MS * 0.001f) + 2;
...
void Flanger::process(float* buffer, int num_samples) {
  ...
  for (int i = 0; i < num_samples; ++i) {
    ...
    const float delayed =
        delay_buffer_[pos0] * (1.0f - frac) + delay_buffer_[pos1] * frac;
...
void Flanger::process_stereo(float* left, float* right, int num_samples) {
  ...
  for (int i = 0; i < num_samples; ++i) {
    ...
    const float delay_samp_l =
        clamp((delay_ms + lfo_l * depth_ms) * 0.001f *
    ...
    const float delayed_l =
        delay_buffer_[ip_l % max_delay_samples_] * (1.0f - f_l) +
        delay_buffer_[(ip_l + 1) % max_delay_samples_] * f_l;

    delay_buffer_[write_pos_] =
        clamp(dry_l + feedback * delayed_l, -2.0f, 2.0f);
    ...
    const float delay_samp_r =
        clamp((delay_ms + lfo_r * depth_ms) * 0.001f *
                  static_cast<float>(sample_rate_),
              1.0f, static_cast<float>(max_delay_samples_ - 2));
    ...
    const float delayed_r =
        delay_buffer_r_[ip_r % max_delay_samples_] * (1.0f - f_r) +
        delay_buffer_r_[(ip_r + 1) % max_delay_samples_] * f_r;

    delay_buffer_r_[write_pos_r_] =
        clamp(dry_r + feedback * delayed_r, -2.0f, 2.0f);
