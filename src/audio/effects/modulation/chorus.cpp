...
Chorus::Chorus() {
  params_ = {{"Rate", 1.5f, 0.1f, 10.0f, 1.5f, "Hz",
              "Speed of the modulation sweep. Higher values create faster "
              "wobbling effects."},
             {"Depth", 5.0f, 0.5f, 20.0f, 5.0f, "ms",
              "Intensity of the modulation. Higher values create a deeper, "
              "more pronounced pitch shift."},
             ...
             {"Subdivision", 2.0f, 0.0f, 3.0f, 2.0f, "",
              "Subdivision: 0=1/1, 1=1/2, 2=1/4, 3=1/8."}};
...
void Chorus::set_transport_state(float bpm) {
  ...
  float subdivision_val = params_[4].value;
  ...
  // set knob
  params_[0].value =
      clamp(target_rate_hz, params_[0].min_val, params_[0].max_val);
