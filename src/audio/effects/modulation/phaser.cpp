...
Phaser::Phaser() {
  params_ = {
      {"Rate", 0.5f, 0.05f, 10.0f, 0.5f, "Hz",
       "LFO speed. Low values create a slow, hypnotic sweep; high values "
       "create a fast "
       "vibrato-like effect."},
      {"Depth", 0.7f, 0.0f, 1.0f, 0.7f, "",
       "Modulation depth. Controls how wide the all-pass frequency sweeps "
       "across the spectrum."},
      {"Stages", 0.0f, 0.0f, 3.0f, 0.0f, "",
       "Number of all-pass stages: 0=4 (Phase 90), 1=6, 2=8, 3=12. More stages "
       "add more "
       "notches."},
      {"Feedback", 0.5f, 0.0f, 0.95f, 0.5f, "",
       "Feeds the chain output back to the input, adding resonance and "
       "intensity to the phasing "
       "notches."},
      {"Mix", 0.5f, 0.0f, 1.0f, 0.5f, "",
       "Dry/wet blend. At 0.5 the notch effect is most pronounced (classic "
       "phaser mix point)."},
  };
...
void Phaser::process(float* buffer, int num_samples) {
  ...
  const int nstages =
      STAGE_COUNTS[(int)clamp(params_[P_STAGES].value + 0.5f, 0.0f, 3.0f)];
  ...
  for (int i = 0; i < num_samples; ++i) {
    ...
    const float fc = clamp(200.0f * std::exp(lfo * depth * log_ratio), 80.0f,
                           static_cast<float>(sample_rate_) * 0.40f);

    // 1st-order all-pass coefficient: c = (tan(π*fc/fs) - 1) / (tan(π*fc/fs) +
    // 1)
    const float t =
        std::tan(3.14159265f * fc / static_cast<float>(sample_rate_));
...
void Phaser::process_stereo(float* left, float* right, int num_samples) {
  ...
  for (int i = 0; i < num_samples; ++i) {
    ...
    const float t_l =
        std::tan(3.14159265f * fc_l / static_cast<float>(sample_rate_));
    ...
    const float fc_r = clamp(200.0f * std::exp(lfo_r * depth * log_ratio),
                             80.0f, static_cast<float>(sample_rate_) * 0.40f);
    const float t_r =
        std::tan(3.14159265f * fc_r / static_cast<float>(sample_rate_));
