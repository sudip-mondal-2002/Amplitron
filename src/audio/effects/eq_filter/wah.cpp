...
WahPedal::WahPedal() {
  params_ = {
      ...
      {"Sweep", 0.5f, 0.0f, 1.0f, 0.5f, "",
       "Manual wah sweep position. Heel-down (0) = low frequency; toe-down (1) "
       "= high frequency. "
       "Active in Manual mode."},
      {"Resonance", 3.5f, 1.0f, 8.0f, 3.5f, "Q",
       "Bandpass filter Q factor. Higher values give a sharper, more vocal wah "
       "character."},
      {"Sensitivity", 0.5f, 0.0f, 1.0f, 0.5f, "",
       "How strongly the input signal amplitude drives the sweep in Auto-wah "
       "mode."},
      {"Attack", 5.0f, 1.0f, 50.0f, 5.0f, "ms",
       "Envelope follower attack time. Faster values track transients more "
       "aggressively."},
      {"Release", 100.0f, 20.0f, 500.0f, 100.0f, "ms",
       "Envelope follower release time. Controls how quickly the filter falls "
       "back after a note "
       "dies."},
  };
}
...
void WahPedal::process(float* buffer, int num_samples) {
  ...
  // Sweep/Q smoothing (~5 ms time constant -- removes zipper noise on knob
  // moves)
