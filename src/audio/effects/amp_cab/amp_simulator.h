...
struct AmpModel {
  ...
  // --- Dynamic response ---
  float
      attack_coeff;  ///< Envelope follower attack speed (0-1, higher = faster)
  float release_coeff;  ///< Envelope follower release speed (0-1, higher =
                        ///< faster)
...
class AmpSimulator : public Effect {
 ...
 private:
  ...
  // One-pole smoothing states for trim parameters (avoids coefficient click on
  // UI change)
