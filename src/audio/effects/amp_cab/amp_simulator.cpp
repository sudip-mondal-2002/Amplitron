...
AmpSimulator::AmpSimulator() {
  ...
  params_ = {
      {"Model", 0.0f, 0.0f, max_model, 0.0f, "",
       "Selects the amplifier model. Each model has unique EQ curves, gain "
       "staging, and clipping "
       "characteristics."},
      {"Gain", 0.5f, 0.0f, 1.0f, 0.5f, "",
       "Preamp gain control. Drives the virtual tubes harder for more "
       "compression and "
       "distortion."},
      {"Bass", 0.0f, -6.0f, 6.0f, 0.0f, "dB",
       "Pre-distortion low-frequency trim. Adjusts the fatness and punch of "
       "the amplifier."},
      {"Mid", 0.0f, -6.0f, 6.0f, 0.0f, "dB",
       "Pre-distortion mid-frequency trim. Controls the core voice and 'bark' "
       "of the amplifier."},
      {"Treble", 0.0f, -6.0f, 6.0f, 0.0f, "dB",
       "Pre-distortion high-frequency trim. Adjusts the brightness and bite "
       "before clipping."},
      {"Level", 0.7f, 0.0f, 1.0f, 0.7f, "",
       "Master output volume of the amplifier. Does not affect the amount of "
       "distortion."},
  };
...
void AmpSimulator::recompute_coefficients_if_dirty() {
  ...
  if (model_idx != cached_model_index_ || bass_trim != cached_bass_ ||
      mid_trim != cached_mid_ || treble_trim != cached_treble_ ||
      gain_knob != cached_gain_) {
    const AmpModel& model = get_amp_models()[model_idx];
    low_shelf_.set_low_shelf(model.bass_freq, model.bass_gain_db + bass_trim,
                             model.bass_q, sample_rate_);
    mid_peak_.set_peaking(model.mid_freq, model.mid_gain_db + mid_trim,
                          model.mid_q, sample_rate_);
    high_shelf_.set_high_shelf(model.treble_freq,
                               model.treble_gain_db + treble_trim,
...
void AmpSimulator::process(float* buffer, int num_samples) {
  ...
  const float alpha =
      1.0f - std::exp(-1.0f / (sample_rate_ * 0.010f));  // 10 ms
