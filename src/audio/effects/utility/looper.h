...
class Looper : public Effect {
 public:
  ...
  State state() const {
    return static_cast<State>(ui_state_.load(std::memory_order_relaxed));
  }
  bool has_loop() const {
    return ui_has_loop_.load(std::memory_order_relaxed) != 0;
  }
  int loop_length_samples() const {
    return ui_loop_length_samples_.load(std::memory_order_relaxed);
  }
  int playhead_samples() const {
    return ui_playhead_samples_.load(std::memory_order_relaxed);
  }
 ...
 private:
  ...
  inline void process_core(float* left, float* right, int num_samples,
                           bool stereo);
};
