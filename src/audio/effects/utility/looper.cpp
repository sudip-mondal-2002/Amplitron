...
Looper::Looper() {
  ...
  loop_level_alpha_ =
      1.0f - std::exp(-1.0f / (sr * kLoopLevelSmoothingSeconds));
...
int Looper::crossfade_samples_rt(float ms) const {
  const int xf = static_cast<int>(
      std::round((ms / 1000.0f) * static_cast<float>(sample_rate_)));
...
void Looper::stop_recording_rt_and_play_rt() {
  ...
  const int min_len = static_cast<int>(
      std::round(kMinLoopSeconds * static_cast<float>(sample_rate_)));
...
void Looper::apply_pending_commands() {
  const uint32_t cmds =
      pending_commands_.exchange(0, std::memory_order_relaxed);
  if (cmds == 0) return;
...
void Looper::process_core(float* left, float* right, int num_samples,
                          bool stereo) {
  apply_pending_commands();
  ...
  crossfade_ms_smoothed_ +=
      crossfade_alpha_ * (crossfade_target_ms - crossfade_ms_smoothed_);
  ...
  if (has_loop_rt_ && loop_length_ > 0 &&
    ...
    for (int i = 0; i < num_samples; ++i) {
      loop_level_smoothed_ +=
          loop_level_alpha_ * (loop_level_target - loop_level_smoothed_);
