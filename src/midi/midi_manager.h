...
class MidiManager : public IMidiManager {
 ...
 public:
  ...
  void remove_mapping_for_param(const std::string&,
  ...
  void start_learn(MidiTargetType, const std::string&,
                   const std::string&) override {}
...
class MidiManager : public IMidiManager {
 ...
 public:
  ...
  /**
   * @brief Enter learn mode: the next CC event received will be bound to the
   * given target.
   */
  ...
  const std::string& learn_effect_name() const override {
    return learn_effect_name_;
  }
  const std::string& learn_param_name() const override {
    return learn_param_name_;
  }
   ...
   * - Otherwise, resolves the mapping target and pushes the value to the
   * engine.
   */
 ...
 private:
  ...
  // Helpers
  void apply_mapping(const MidiMapping& mapping, int cc_value,
                     IAudioEngine& engine);
