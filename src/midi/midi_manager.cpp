...
void (*g_mock_rtmidi_callback)(double, std::vector<unsigned char>*,
                               void*) = nullptr;
...
class MockRtMidiIn {
 public:
  MockRtMidiIn(int = 0, const std::string& = "") {
    if (g_mock_rtmidi_should_fail_constructor)
      throw RtMidiError("Mock constructor failure");
  }
  void ignoreTypes(bool, bool, bool) {}
  unsigned int getPortCount() {
    if (g_mock_rtmidi_should_fail_enumerate)
      throw RtMidiError("Mock port count failure");
  ...
  void setCallback(void (*cb)(double, std::vector<unsigned char>*, void*),
                   void* data) {
  ...
  void closePort() {
    if (g_mock_rtmidi_should_fail_close)
      throw RtMidiError("Mock close failure");
...
bool MidiManager::open_port(int port_index) {
  ...
  try {
    unsigned int count = rt->getPortCount();
    if (port_index < 0 || static_cast<unsigned int>(port_index) >= count)
      return false;
