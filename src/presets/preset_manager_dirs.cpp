...
std::string PresetManager::get_user_presets_dir() {
  char* result = (char*)EM_ASM_PTR(
      { return stringToNewUTF8(window._amplitronPresetDir || 'preset'); });
...
std::string get_bundled_presets_dir() {
  ...
  if (_NSGetExecutablePath(exe_path, &size) == 0) {
    ...
    if (last_slash != std::string::npos) {
      std::string bundle_presets =
          exe_str.substr(0, last_slash) + "/../Resources/presets";
...
#ifdef _WIN32
  // Allow callers/tests to pass forward slashes; normalize for
  // _findfirst/_mkdir.
...
void PresetManager::save_factory_presets(const std::string& dir) {
  ...
  if (!dir_exists(src_dir)) {
    std::cerr << "Bundled presets directory not found: " << src_dir
              << std::endl;
    return;
  }
