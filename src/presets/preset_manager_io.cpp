  ...
  if (preset.routing == "graph") {
    ...
    for (auto &fd : preset.effects) {
      ...
      if (it != fd.metadata.end() && !it->second.empty()) {
        ...
        if (cab) {
          if (!is_safe_ir_path(it->second)) {
            std::cerr << "Cabinet: preset contains unsafe IR path, skipping IR load: "
                      << it->second << std::endl;
          } else if (!cab->load_ir(it->second)) {
            std::cerr << "Cabinet: could not load IR file: " << it->second << std::endl;
          }
        }
      }
