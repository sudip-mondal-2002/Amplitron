...
std::string PresetManager::apply_migrations(
  ...
  bool is_root_version =
      (version_pos != std::string::npos && (version_pos - root_start) < 100);

  if (!is_root_version) {
    std::cout << "[Preset Migration] Upgrading legacy unversioned preset "
                 "format to Version "
    ...
    if (last_bracket != std::string::npos && last_bracket > root_start) {
      // Find out if there is any content between the root brackets to avoid
      // trailing comma bugs
      size_t content_check =
          patched.find_first_not_of(" \t\n\r", root_start + 1);
      bool is_empty_json = (content_check == last_bracket);

      // Construct the exact version upgrade block using our header constant
      // dynamically
      ...
      migration_patch +=
          ...
          ",\n";
      migration_patch +=
          "  \"version\": " + std::to_string(CURRENT_PRESET_VERSION) + ",\n";
