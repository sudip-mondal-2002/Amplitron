...
TEST(spectrum_analyzer_construction_initializes_floor) {
  ...
  sa.draw(nullptr, ImVec2(0, 0), ImVec2(100, 100),
          SpectrumAnalyzer::DisplayMode::Output);
}
...
TEST(spectrum_analyzer_update_tone_does_not_produce_nan) {
  ...
  sa.draw(nullptr, ImVec2(0, 0), ImVec2(800, 200),
          SpectrumAnalyzer::DisplayMode::Input);
}
...
TEST(spectrum_analyzer_draw_null_drawlist_is_safe_noop) {
  ...
  sa.draw(nullptr, ImVec2(0, 0), ImVec2(400, 100),
          SpectrumAnalyzer::DisplayMode::Input);
  sa.draw(nullptr, ImVec2(0, 0), ImVec2(400, 100),
          SpectrumAnalyzer::DisplayMode::Output);
  sa.draw(nullptr, ImVec2(0, 0), ImVec2(400, 100),
...
TEST(spectrum_analyzer_draw_zero_size_is_safe_noop) {
  SpectrumAnalyzer sa;
  // size.x <= 2 or size.y <= 2 should early-return even with a non-null
  // draw_list We pass nullptr since we don't have a real draw list in headless
  // mode
  sa.draw(nullptr, ImVec2(10, 10), ImVec2(1, 1),
          SpectrumAnalyzer::DisplayMode::Output);
  sa.draw(nullptr, ImVec2(10, 10), ImVec2(0, 200),
          SpectrumAnalyzer::DisplayMode::Output);
  sa.draw(nullptr, ImVec2(10, 10), ImVec2(200, 0),
          SpectrumAnalyzer::DisplayMode::Output);
}
...
TEST_F(PresetTest, gui_presets_begin_new_preset_resets_state) {
  ...
  gp.begin_new_preset();
  // After begin_new_preset, selected index should reset to -1 and name should
  // be empty
  ASSERT_EQ(gp.selected_preset_index(), -1);
}
...
TEST_F(AudioEngineTest, gui_analyzer_reserved_height_values) {
  ...
  // When expanded, height = 245; when collapsed (can't toggle without ImGui),
  // height = 38 We can at least verify the expanded value is positive and
  // reasonable
