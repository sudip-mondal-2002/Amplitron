 ...
 * @brief Headless-safe tests for PedalBoard and PedalWidget logic and
 * rendering.
 *
 * Covers: widget rebuild logic, active-only filtering, AmpSimulator detection,
 * PedalWidget index/effect/history accessors, and ImGui rendering for all pedal
 * visual types (standard, Amp, Tuner, Cabinet, Looper, and MultiBand
 * Compressor) using a software ImGui context.
 */
...
TEST(pedal_board_rebuild_with_amp_simulator) {
  ...
  // Post-amp effects are excluded from visible indices — board must remain
  // stable
...
TEST(pedal_widget_body_and_knob_adjustments) {
  ...
  widget.render_standard_pedal(dl, ImVec2(0, 0), ImVec2(200, 300), 200.0f, true,
                               1.0f);
