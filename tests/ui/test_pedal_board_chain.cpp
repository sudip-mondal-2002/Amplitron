...
static ImGuiID get_item_id(const char* window_substr, const char* item_id_str) {
  ...
  snprintf(popup_window_name, sizeof(popup_window_name), "##Popup_%08x",
           popup_id);
...
TEST_F(PresetTest, test_pedal_board_chain_scrolling_and_zooming) {
  ...
  // MousePos is stateful (set before advance_frame), MouseWheel is
  // instantaneous (set after advance_frame)
  io.MousePos = ImVec2(512, 384);
  advance_frame();
  io.KeyCtrl = true;
  io.MouseWheel = 1.0f;
...
TEST_F(PresetTest, test_implicit_hand_tool) {
  ...
  io.MouseDelta = ImVec2(
      -20, -10);  // simulate movement delta (EndFrame zeroed MousePosPrev)
  TestAccessor::render_signal_chain(board);
...
TEST_F(PresetTest, test_pedal_board_chain_nodes_and_wiring) {
  ...
  if (deletable_node_id != -1) {
    io.MousePos = ImVec2(100.0f + 190.0f - 12.0f,
                         100.0f + 10.0f);  // Approximate cross button location
    advance_frame();
    io.MouseDown[0] = true;
    TestAccessor::render_signal_chain(board);
...
TEST_F(PresetTest, test_pedal_board_chain_extended) {
  ...
  ImVec2 splitter_pin_pos(canvas_pos.x + 300.0f + 130.0f + 2.0f,
                          canvas_pos.y + 100.0f + 39.67f);
  ImVec2 mixer_pin_pos(canvas_pos.x + 500.0f - 2.0f,
                       canvas_pos.y + 100.0f + 39.67f);
  ...
  for (const auto& link : audio_graph.get_links()) {
    if (link.source_pin_id == splitter_out_pin &&
        link.dest_pin_id == mixer_in_pin) {
      link_found = true;
      break;
    }
  }
  ...
  // 3. Test link deletion via bezier hover (click on the splitter output pin
  ...
  ImVec2 close_pos(canvas_pos.x + 300.0f + 130.0f - 12.0f,
                   canvas_pos.y + 100.0f + 14.0f);
...
TEST_F(PresetTest, test_pedal_board_chain_fully_covered) {
  ...
  for (const auto& node : audio_graph.get_nodes()) {
    if (node.name == "Input") {
      ui_state.node_positions[node.id] = {ImVec2(40.0f, 100.0f), false,
                                          ImVec2(0.0f, 0.0f)};
    } else if (node.pedal == od) {
      ui_state.node_positions[node.id] = {ImVec2(200.0f, 100.0f), false,
                                          ImVec2(0.0f, 0.0f)};
    } else {
      ui_state.node_positions[node.id] = {ImVec2(900.0f, 100.0f), false,
                                          ImVec2(0.0f, 0.0f)};
    }
  }
  ...
  ui_state.node_positions[splitter_id] = {ImVec2(450.0f, 100.0f), false,
                                          ImVec2(0.0f, 0.0f)};
  ui_state.node_positions[mixer_id] = {ImVec2(650.0f, 100.0f), false,
                                       ImVec2(0.0f, 0.0f)};
  ui_state.node_positions[mb_comp_id] = {ImVec2(900.0f, 100.0f), false,
                                         ImVec2(0.0f, 0.0f)};
  ...
  ImVec2 drag_pos(canvas_pos.x + od_pos.x + 50.0f,
                  canvas_pos.y + od_pos.y + 15.0f);
  ...
  // 2.5 Drag utility node with exact coordinates (vertical-only to cover
  // short-circuit on y offset check)
  ImVec2 split_pos = ui_state.node_positions[splitter_id].position;
  ImVec2 split_drag_pos(canvas_pos.x + split_pos.x + 50.0f,
                        canvas_pos.y + split_pos.y + 15.0f);
  ...
  // Overdrive starts at 200, width = 190. Close button is at X: 200 + 190 - 24
  // = 366
