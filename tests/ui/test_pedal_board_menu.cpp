...
TEST_F(PresetTest, test_pedal_board_menu_extended) {
  ...
  // 1. Test auto-add amp simulator path in render_amp_selector when engine has
  // no effects
  ...
  const std::vector<std::string> effects_to_add = {
      "Overdrive",
      "Distortion",
      "Noise Gate",
      "Compressor",
  ...
  const std::vector<std::string> amp_models = {
      "Clean American", "British Crunch", "High Gain Modern", "Jazz Warm"};
  ...
  // 4. Test disconnected MIDI state in MIDI menu by clicking MIDI button
  // programmatically
  TestAccessor::learn_active(midi_manager) = false;
