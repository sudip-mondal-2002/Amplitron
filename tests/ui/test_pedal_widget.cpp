...
TEST_F(PresetTest, test_pedal_widget_color_assignment_all_types) {
  ...
  std::vector<std::pair<std::shared_ptr<Effect>, std::string>> test_cases = {
      ...
      {std::make_shared<WahPedal>(),
       "Default"}  // Wah should fall back to Default color
  };
...
TEST_F(PresetTest, test_pedal_widget_footswitch_click_interaction) {
  ...
  // From p0 = (10, 10), pedal_width = 190.0f, pedal_height = 360.0f,
  // SWITCH_BOTTOM_OFFSET = 55.0f: switch_y = 10 + 360 - 55 = 315 switch_x = 10
  // + (190 - 50) / 2 = 80 sw_center = (80 + 25, 315 + 15) = (105, 330)
  ...
  TestAccessor::render_footswitch_and_extras(widget, dl, p0, p1, 190.0f, 360.0f,
                                             false, true, should_remove, 1.0f);
  advance_frame();
