...
TEST_F(PresetTest, test_knob_component_comprehensive) {
  ...
  // Call render again when popup is open so that the popup content is actually
  // rendered!
  KnobComponent::render("Knob1", props, 1.0f, center);
  advance_frame();
...
TEST_F(PresetTest,
       KnobComponent_DoubleClick_ResetsToDefault_CallsBothCallbacks) {
  ScopedImGuiContext imgui;
  ImGui::SetNextWindowPos(ImVec2(0, 0));
  ImGui::SetNextWindowSize(ImVec2(1024, 768));
  ImGui::Begin("TestWindow");
...
TEST_F(PresetTest,
       KnobComponent_DoubleClick_WhenValueAlreadyDefault_NoCallbackFired) {
  ScopedImGuiContext imgui;
  ImGui::SetNextWindowPos(ImVec2(0, 0));
  ImGui::SetNextWindowSize(ImVec2(1024, 768));
  ImGui::Begin("TestWindow");
...
TEST_F(PresetTest, KnobComponent_Popup_SliderInteractions) {
  ...
  // Trigger learning callbacks explicitly since we can't easily click menu
  // items inside popup
  props.on_midi_learn_param();
  props.on_midi_clear_param();
  props.on_midi_learn_bypass();
...
TEST_F(PresetTest, KnobComponent_PopupMidiLearnItems) {
  ...
  ImGuiID learn_b_id =
      get_popup_item_id("Popup_KMidi", "MIDI Learn Bypass Toggle");
  ...
  ImGuiID remove_b_id =
      get_popup_item_id("Popup_KMidi", "Remove Bypass Mapping");
