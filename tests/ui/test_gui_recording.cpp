 ...
 * states (Ready, Recording, Unsaved, and Save dialog triggers) using a
 * software-only ImGui context.
 */
#include <string>
...
TEST_F(PresetTest, gui_recording_render_save_dialog_triggers_safely) {
  ...
  // In headless test environment, the native dialog handles gracefully and
  // should clear needs_save.
  ASSERT_FALSE(gr.needs_save_dialog());
}
