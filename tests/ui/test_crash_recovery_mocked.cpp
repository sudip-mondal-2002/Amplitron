...
inline int mock_SDL_ShowMessageBox(const SDL_MessageBoxData* /*messageboxdata*/,
                                   int* buttonid) {
...
// gui/crash_recovery_ui.h so the macro redirection applies, and
// crash_recovery_ui.h must not re-include SDL.h in a way that bypasses the
// macros; also note that the test undefines AMPLITRON_HEADLESS to exercise
// non-headless code paths.
...
TEST(crash_recovery_already_initialized_restore) {
  ...
  ASSERT_FALSE(
      g_mock_sdl_init_subsystem_called);  // should not initialize again
