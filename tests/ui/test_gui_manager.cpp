...
struct GuiManagerTestAccessor {
  ...
  static void toggle_audio_mute_state(GuiManager& g) {
    g.toggle_audio_mute_state();
  }
  static void render_master_controls(GuiManager& g) {
    g.render_master_controls();
  }
  static void render_menu_bar(GuiManager& g) { g.render_menu_bar(); }
  static RecordingProps build_recording_props(GuiManager& g) {
    return g.build_recording_props();
  }
  static TunerProps build_tuner_props(GuiManager& g) {
    return g.build_tuner_props();
  }
  static SettingsProps build_settings_props(GuiManager& g) {
    return g.build_settings_props();
  }
  static AnalyzerProps build_analyzer_props(GuiManager& g) {
    return g.build_analyzer_props();
  }
  static SnapshotsProps build_snapshots_props(GuiManager& g) {
    return g.build_snapshots_props();
  }
  static AudioMetricsService& get_metrics_service(GuiManager& g) {
    return g.metrics_service_;
  }
};
}  // namespace Amplitron
...
TEST(amplitron_session_throws_on_null_arguments) {
  ...
  ASSERT_THROW(AmplitronSession(std::make_unique<AudioEngine>(), nullptr,
                                std::make_unique<PresetManagerService>()),
               std::invalid_argument);
  ASSERT_THROW(AmplitronSession(std::make_unique<AudioEngine>(),
                                std::make_unique<MidiManager>(), nullptr),
               std::invalid_argument);
}
...
TEST(gui_manager_private_rendering_methods) {
  ...
  engine.set_running_for_testing(
      false);  // Headless-safe: manually update engine state since Pa_Stream is
               // nullptr
  GuiManagerTestAccessor::toggle_audio_mute_state(gui);
...
TEST(gui_manager_logical_builders) {
  ...
  // 4. build_analyzer_props
  {
    ...
    ASSERT_TRUE(p.spectrum.smoothed_input_db ==
                GuiManagerTestAccessor::get_metrics_service(gui)
                    .spectrum_analyzer()
                    .smoothed_input_db());
...
struct MockDrawList {
  ...
  void AddRect(const ImVec2&, const ImVec2&, unsigned int, float = 0.0f,
               int = 0, float = 1.0f) {}
};
...
struct MockImGui {
  ...
  static void EndMenu() {}
  static bool MenuItem(const char* label, const char* = nullptr, bool = false,
                       bool = true) {
  ...
  static ImVec2 CalcTextSize(const char*, const char* = nullptr, bool = false,
                             float = -1.0f) {
  ...
  static bool BeginChild(const char*, const ImVec2& = ImVec2(0, 0),
                         bool = false, int = 0) {
  ...
  static void End() {}
  static void SetNextWindowPos(const ImVec2&, int = 0,
  ...
  static bool SliderFloat(const char*, float*, float, float,
                          const char* = nullptr, float = 1.0f) {
    return slider_float_ret;
  }
  static inline bool slider_int_ret = false;
  static bool SliderInt(const char*, int*, int, int, const char* = nullptr,
                        int = 0) {
...
struct MockPresetsWrapper {
  ...
  std::string current_preset_name() const {
    return real_presets.current_preset_name();
  }
  ...
  int selected_preset_index() const {
    return real_presets.selected_preset_index();
  }
  int preset_count() const { return real_presets.preset_count(); }
  bool delete_preset_by_index(int index) {
    return real_presets.delete_preset_by_index(index);
  }
...
struct MockKeyboardShortcutsWrapper {
  ...
  void set_props(const Amplitron::KeyboardShortcutsProps& p) {
    real_shortcuts.set_props(p);
  }
};
...
inline char* mock_fgets(char* str, int num, FILE*) {
  ...
  size_t len = 0;
  while (mock_popen_pos < mock_popen_result.size() &&
         len < static_cast<size_t>(num - 1)) {
...
TEST(gui_manager_additional_coverage_run) {
  ...
  // 2. gui_manager.cpp Lines 66-67 (default MIDI mappings when load config is
  // empty)
  {
#ifdef _WIN32
    ...
    std::cout << "[DEBUG] OLD " << env_name << ": "
              << (old_val ? old_val : "NULL") << std::endl;
#ifdef _WIN32
    ...
    std::cout << "[DEBUG] NEW " << env_name << ": "
              << (std::getenv(env_name) ? std::getenv(env_name) : "NULL")
              << std::endl;
    std::cout << "[DEBUG] CONFIG PATH: " << MidiManager::get_config_path()
              << std::endl;

    Amplitron::AmplitronSession session;
    Amplitron::GuiManager gui(session);
    std::cout << "[DEBUG] MAPPINGS SIZE BEFORE CLEAR: "
              << gui.midi_manager().mappings().size() << std::endl;
    gui.midi_manager().clear_mappings();
    std::cout << "[DEBUG] MAPPINGS SIZE AFTER CLEAR: "
              << gui.midi_manager().mappings().size() << std::endl;
    gui.initialize();
    std::cout << "[DEBUG] MAPPINGS SIZE AFTER INIT: "
              << gui.midi_manager().mappings().size() << std::endl;
  ...
  // 5. gui_manager_frame.cpp Lines 272-276 (save recorder callback)
  {
    ...
    gui.gui_recording_.set_state(
        [](RecordingState& s) { s.needs_save = true; });
