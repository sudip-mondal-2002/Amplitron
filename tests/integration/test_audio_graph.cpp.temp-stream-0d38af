...
TEST(audio_graph_parallel_split_merge) {
  ...
  // Link splits: Pedal 1 splits directly into both 2 and 3 using separate
  // output pins
  int l1 =
...
TEST(audio_graph_rapid_graph_rebuild_stress) {
  ...
  for (int iteration = 0; iteration < 100; ++iteration) {
    ...
    ASSERT_TRUE(graph.add_link(nodes[0].output_pin_ids[0],
                               nodes[1].input_pin_ids[0]) != -1);

    ASSERT_TRUE(graph.add_link(nodes[0].output_pin_ids[1],
                               nodes[2].input_pin_ids[0]) != -1);

    ASSERT_TRUE(graph.add_link(nodes[1].output_pin_ids[0],
                               nodes[3].input_pin_ids[0]) != -1);

    ASSERT_TRUE(graph.add_link(nodes[2].output_pin_ids[0],
...
TEST(audio_graph_link_enforcement) {
  ...
  int l4 =
      graph.add_link(nodes[2].output_pin_ids[0], nodes[1].input_pin_ids[0]);
  ASSERT_TRUE(l4 == -1);
}
...
TEST(audio_graph_remove_node_and_link) {
  ...
  ASSERT_TRUE(graph.get_links().size() ==
              0);  // Link connected to n2 should be removed
}
...
TEST(audio_graph_input_pin_already_used_rejection) {
  ...
  int invalid =
      graph.add_link(nodes[0].output_pin_ids[1], nodes[1].input_pin_ids[0]);
...
TEST(audio_graph_mixer_gains) {
  ...
  nodes = graph.get_nodes();
  graph.add_link(nodes[0].output_pin_ids[0],
                 nodes[2].input_pin_ids[0]);  // p1[0] -> path1
  graph.add_link(nodes[0].output_pin_ids[1],
                 nodes[3].input_pin_ids[0]);  // p1[1] -> path2
  graph.set_node_as_input(path3, true);

  nodes = graph.get_nodes();
  graph.add_link(nodes[2].output_pin_ids[0],
                 nodes[1].input_pin_ids[0]);  // path1 -> mixer[0]
  graph.add_link(nodes[3].output_pin_ids[0],
                 nodes[1].input_pin_ids[1]);  // path2 -> mixer[1]
  graph.add_link(nodes[4].output_pin_ids[0],
                 nodes[1].input_pin_ids[2]);  // path3 -> mixer[2]
...
TEST(audio_graph_split_merge_disconnects) {
  ...
  // splitter -> B
  int l_split_b =
      graph.add_link(nodes[1].output_pin_ids[1], nodes[3].input_pin_ids[0]);

  // A -> Merge
  int l_a_merge =
      graph.add_link(nodes[2].output_pin_ids[0], nodes[4].input_pin_ids[0]);
...
TEST(audio_graph_extra_branches) {
  ...
  graph.restore_link(link1);  // Restoring a link that creates a cycle (source
                              // output -> self input)
  ...
  int fx_node =
      graph2.add_node("Overdrive", NodeRoutingType::StandardEffect, effect);
