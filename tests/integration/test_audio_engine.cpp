...
TEST_F(AudioEngineTest, MetronomeState) {
  ...
  // Process audio to cover the metronome click generation in
  // audio_engine_process.cpp
  std::vector<float> in(64, 0.0f), out(128, 0.0f);
...
TEST_F(AudioEngineTest, CopyAnalyzerSnapshot) {
  ...
  bool success =
      engine.copy_analyzer_snapshot(snap_in.data(), snap_out.data(), 1024);
  ASSERT_EQ(success, true);
  ASSERT_NEAR(snap_in[0], 0.5f, 0.01f);

  // Test analyzer copy error conditions for branch coverage
  ASSERT_EQ(engine.copy_analyzer_snapshot(nullptr, snap_out.data(), 1024),
            false);
  ASSERT_EQ(engine.copy_analyzer_snapshot(snap_in.data(), nullptr, 1024),
            false);
  ASSERT_EQ(engine.copy_analyzer_snapshot(snap_in.data(), snap_out.data(), -1),
            false);

  AudioEngine new_engine;
  ASSERT_EQ(
      new_engine.copy_analyzer_snapshot(snap_in.data(), snap_out.data(), 1024),
      false);  // seq == 0
}
...
TEST_F(AudioEngineTest, TunerTap) {
  ...
  // Process audio to cover the tuner tap execution branch in
  // audio_engine_process.cpp
...
TEST_F(AudioEngineTest, CommandQueuePushes) {
  ...
  engine.set_input_gain(0.5f);  // this gets stuck behind effect commands and
                                // handled by drain_commands
...
TEST_F(AudioEngineTest, RecorderBranch) {
  ...
  std::string temp_file = (std::filesystem::temp_directory_path() /
                           ("test_record_" + std::to_string(now) + ".wav"))
                              .string();
...
TEST_F(AudioEngineTest, PushMixerGainChangeDrainsSuccessfully) {
  int n1 = engine.graph().add_node("SourceNode",
                                   NodeRoutingType::StandardEffect, nullptr);
  int mixer_id =
      engine.graph().add_node("MixerNode", NodeRoutingType::Mixer, nullptr);

  // Link SourceNode to MixerNode so that MixerNode has an input source in
  // executor step
...
TEST_F(AudioEngineTest,
  ...
  ASSERT_GE(engine.test_process_buffer().size(),
            static_cast<size_t>(large_frame_count));
  ASSERT_GE(engine.test_process_buffer_right().size(),
            static_cast<size_t>(large_frame_count));
}
