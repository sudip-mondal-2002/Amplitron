...
TEST(recorder_wav_header_riff_size_field_is_correct) {
  ...
  uint32_t expected_riff_size =
      static_cast<uint32_t>(num_samples * 1 * 2) + 36u;
...
TEST(recorder_stereo_write_samples_stereo_interleaves_correctly) {
  ...
  // File size should be: 44 bytes header + (256 frames * 2 channels * 2
  // bytes/sample) = 44 + 1024 = 1068 bytes
  ...
  // Verify samples are interleaved: left (index 0, positive) and right (index
  // 1, negative)
...
TEST(recorder_start_invalid_params_fails) {
  ...
  ASSERT_FALSE(
      rec.start("recordings/test_not_a_dir.txt/recording.wav", 48000, 1));
  ASSERT_FALSE(rec.is_recording());
