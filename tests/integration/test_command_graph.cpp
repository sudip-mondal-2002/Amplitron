...
TEST(AddGraphNodeCommand_executes_and_undoes_correctly) {
  ...
  auto cmd = std::make_unique<AddGraphNodeCommand>(
      engine, "TestNode", NodeRoutingType::Splitter, nullptr, ImVec2(100, 100));
  ...
  ASSERT_EQ(100.0f,
            GuiGraphState::get_instance().node_positions[node_id].position.x);
...
TEST(AddGraphNodeCommand_skips_position_tracking_on_zero_coords) {
  ...
  auto cmd = std::make_unique<AddGraphNodeCommand>(
      engine, "AutoNode", NodeRoutingType::StandardEffect, nullptr,
      ImVec2(0, 0));
  history.execute(std::move(cmd));
...
TEST(RemoveGraphNodeCommand_stores_severed_links_and_restores_them_on_undo) {
  ...
  auto cmd = std::make_unique<RemoveGraphNodeCommand>(
      engine, n1, NodeRoutingType::Splitter, ImVec2(0, 0));
  history.execute(std::move(cmd));
...
TEST(MoveGraphNodeCommand_executes_and_undoes_correctly) {
  ...
  auto cmd = std::make_unique<MoveGraphNodeCommand>(node_id, ImVec2(10, 10),
                                                    ImVec2(50, 50));
  history.execute(std::move(cmd));
...
TEST(RemoveGraphNodeCommand_fails_on_missing_node_and_leaves_history_clean) {
  ...
  auto cmd = std::make_unique<RemoveGraphNodeCommand>(
      engine, 9999, NodeRoutingType::Splitter, ImVec2(0, 0));
  history.execute(std::move(cmd));
...
TEST(MoveGraphNodeCommand_fails_on_missing_node_or_identical_position) {
  ...
  auto cmd1 = std::make_unique<MoveGraphNodeCommand>(8888, ImVec2(0, 0),
                                                     ImVec2(10, 10));
  history.execute(std::move(cmd1));
  ASSERT_EQ(0, history.undo_size());
  ...
  auto cmd2 = std::make_unique<MoveGraphNodeCommand>(node_id, ImVec2(50, 50),
                                                     ImVec2(50, 50));
  history.execute(std::move(cmd2));
  ASSERT_EQ(0, history.undo_size());
...
TEST(CommandGraph_DynamicPinAndDescription) {
  ...
  AddGraphNodeCommand cmd_add(engine, "Test", NodeRoutingType::Splitter,
                              nullptr, ImVec2(0, 0));
  ASSERT_EQ(std::string("Add Node"), cmd_add.description());

  RemoveGraphNodeCommand cmd_rem(engine, 1, NodeRoutingType::Splitter,
                                 ImVec2(0, 0));
  ...
  ASSERT_EQ(graph.find_node(s1)->output_pin_ids.size(), 2u);
  ASSERT_EQ(graph.find_node(m1)->input_pin_ids.size(), 2u);
  ...
  auto link1 = std::make_unique<AddGraphLinkCommand>(
      engine, graph.find_node(s1)->output_pin_ids[0],
  ...
  auto link2 = std::make_unique<AddGraphLinkCommand>(
      engine, graph.find_node(s1)->output_pin_ids[1],
  ...
  auto link4 = std::make_unique<AddGraphLinkCommand>(
      engine, graph.find_node(n2)->output_pin_ids[0],
  ...
  // Redo Link 4
  history.redo();
  ASSERT_EQ(graph.find_node(m1)->input_pin_ids.size(), 3u);

  // 5. Test removing link auto-removes empty pin
  // We have 3 output pins on S1 and 3 input pins on M1. Let's link S1(outC) ->
  // N3(in) S1 has 3 output pins, 2 are linked. Occupied count will become 3
  // after linking outC. So Splitter output pins will grow to 4.
  auto link5 = std::make_unique<AddGraphLinkCommand>(
      engine, graph.find_node(s1)->output_pin_ids[2],
  ...
  // Removing link should auto-remove the empty pin, shrinking Splitter output
  // pins back to 3
  ASSERT_EQ(graph.find_node(s1)->output_pin_ids.size(), 3u);
  ...
  // 6. Test RemoveGraphNodeCommand auto-removes empty Mixer pins and restores
  // on Undo First, let's link N3(out) -> M1(inC) M1 has 3 input pins, 2 are
  // linked (N1, N2). Linking N3 will occupy all 3, growing Mixer inputs to 4.
  auto link6 = std::make_unique<AddGraphLinkCommand>(
      engine, graph.find_node(n3)->output_pin_ids[0],
      graph.find_node(m1)->input_pin_ids[2]);
  history.execute(std::move(link6));
  ASSERT_EQ(graph.find_node(m1)->input_pin_ids.size(), 4u);

  // Now remove node N3. This should sever link N3(out) -> M1(inC), leaving M1
  // with an empty pin. Since M1 input count is > 2, it should auto-remove the
  // empty pin, shrinking Mixer inputs back to 3.
  auto rem_node = std::make_unique<RemoveGraphNodeCommand>(
      engine, n3, NodeRoutingType::StandardEffect, ImVec2(0, 0));
  history.execute(std::move(rem_node));

  // Verify node N3 is gone and Mixer inputs shrunk to 3
  ASSERT_EQ(graph.find_node(m1)->input_pin_ids.size(), 3u);

  // Undo node removal should restore N3, the link N3 -> M1, and Mixer inputs
  // back to 4
  history.undo();
  ASSERT_EQ(graph.find_node(m1)->input_pin_ids.size(), 4u);
