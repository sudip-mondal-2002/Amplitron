...
TEST(presets_dirty_flag_input_gain) {
  ...
  bool is_dirty =
      !equal_preset_data(saved_state, capture_current_state(engine));
...
TEST(preset_migration_load_v1) {
  ...
  std::string upgraded_payload =
      PresetManager::apply_migrations(legacy_v1_payload);
  ...
  ASSERT_TRUE(upgraded_payload.find("\"input_gain\": 0.7") !=
              std::string::npos);
}
...
TEST(preset_migration_load_v2) {
  ...
  std::string processed_payload =
      PresetManager::apply_migrations(modern_v2_payload);
