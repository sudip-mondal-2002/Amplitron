...
static void setup_mocks() {
  ...
  Amplitron::g_mock_pa_host_api_device_index_to_device_index = [](int, int d) {
    return d;
  };
  Amplitron::g_mock_pa_get_device_info = [](int idx) -> const PaDeviceInfo* {
    if (idx == 0) return &mock_info_usb_in;
    if (idx == 1) return &mock_info_out;
    if (idx == 2) return &mock_info_normal_in;
    return nullptr;
  };
  Amplitron::g_mock_pa_get_default_input_device = []() { return 0; };
  Amplitron::g_mock_pa_get_default_output_device = []() { return 1; };
  Amplitron::g_mock_pa_open_stream =
      [](PaStream** stream, const PaStreamParameters*,
  ...
  Amplitron::g_mock_pa_stop_stream = [](PaStream*) -> PaError {
    return paNoError;
  };
  Amplitron::g_mock_pa_close_stream = [](PaStream*) -> PaError {
    return paNoError;
  };
}
...
TEST(PortAudioDevices_SetDeviceBranchCoverage) {
  ...
  if (!in_devs.empty() && !out_devs.empty()) {
    ...
    engine_uninit.set_output_device(
        valid_out);  // also hits !share when input was just set
  }
...
class PortAudioTestSaboteur {
 public:
  static void sabotage_and_test() {
    ...
    if (!in_devs.empty() && !out_devs.empty()) {
      if (engine.start()) {
        ...
        // Try to set input device. It will stop, set, and then try to start()
        // again. start() will immediately return false because initialized_ is
        // false. It will then hit the revert logic and fail to revert too
        // (since initialized_ is still false).
  ...
  static void success_device_set() {
    ...
    g_mock_pa_get_host_api_info = [](int) -> const PaHostApiInfo* {
      return &mock_api_info;
    };
  ...
  static void devices_coverage() {
    ...
    // set_input_device -> stop() -> start() [fails, start_calls=2] -> revert ->
    // start() [succeeds, start_calls=3]
    start_calls = 1;
...
TEST(PortAudioLifecycle_MockAutoDetectSystemDefault) {
  ...
  Amplitron::g_mock_pa_host_api_device_index_to_device_index = [](int, int) {
    return -1;
  };
...
TEST(PortAudioLifecycle_MockInitializeFail) {
  MockGuard guard;
  Amplitron::g_mock_pa_initialize = []() -> PaError {
...
TEST(PortAudioBackend_ExtraCoverage) {
  ...
  g_mock_pa_get_device_info = [](int) -> const PaDeviceInfo* {
    return nullptr;
  };
