...
public:
  ...
  static void success_device_set() {
    ...
    g_mock_pa_open_stream = [](PaStream **stream, const PaStreamParameters *,
                               const PaStreamParameters *, double,
                               unsigned long, PaStreamFlags, PaStreamCallback *,
                               void *) -> PaError {
  ...
  static void devices_coverage() {
    ...
    g_mock_pa_get_device_info = [](int i) -> const PaDeviceInfo * {
      if (i == 0)
        return nullptr;
      if (i == 1)
        return &mock_info_out; // 0 input, 2 output
      if (i == 2)
        return &mock_info_usb_in; // 2 input, 0 output
      return nullptr;
    };
...
TEST(PortAudioLifecycle_MockAutoDetectUSB) {
  ...
  Amplitron::g_mock_pa_get_device_info = [](int i) -> const PaDeviceInfo * {
    ...
    if (i == 1)
      return &mock_info_out;
    if (i == 2)
      return &mock_info_normal_in;
    return nullptr;
  };
...
TEST(PortAudioLifecycle_MockStartSampleRateMismatch) {
  ...
  Amplitron::g_mock_pa_get_stream_info =
      [](PaStream *) -> const PaStreamInfo * { return &mock_stream_info; };
  Amplitron::g_mock_pa_get_device_info = [](int) -> const PaDeviceInfo * {
    return &mock_info_usb_in;
  };
