...
TEST(json_preset_roundtrip_via_string) {
    ...
    ASSERT_EQ(restored.effects[2].metadata.at("ir_path"), std::string("/some/path/cabinet.wav"));
...
TEST(json_preset_signal_chain_dump) {
    ...
    for (size_t i = 0; i < state.effects.size(); ++i) {
        ...
        ASSERT_EQ(restored.effects[i].params.size(), state.effects[i].params.size());
        for (size_t p = 0; p < state.effects[i].params.size(); ++p) {
            ASSERT_EQ(restored.effects[i].params[p].first, state.effects[i].params[p].first);
            ASSERT_NEAR(restored.effects[i].params[p].second, state.effects[i].params[p].second,
                        0.001f);
        }
    }
...
TEST(json_from_json_exceptions) {
    nlohmann::json j1 = {{"routing", "graph"}, {"links", nlohmann::json::array()}};
    PresetData p1;
    bool caught1 = false;
    try {
        from_json(j1, p1);
    ...
    nlohmann::json j2 = {{"routing", "graph"}, {"nodes", nlohmann::json::array()}};
    PresetData p2;
    bool caught2 = false;
    try {
        from_json(j2, p2);
