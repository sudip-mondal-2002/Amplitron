...
static void setup_main_pa_mocks() {
  ...
  g_mock_pa_get_host_api_info = [](int) -> const PaHostApiInfo* {
    return &mock_main_api;
  };
  g_mock_pa_host_api_device_index_to_device_index = [](int, int d) {
    return d;
  };
...
TEST(main_headless_full_execution) {
  ...
  char* argv[] = {
      (char*)"amplitron", (char*)"--preset",  (char*)preset_path.c_str(),
  ...
  std::thread main_thread(
      [&exit_code, &argv]() { exit_code = app_main(8, argv); });
...
TEST(main_headless_preset_not_found) {
  char* argv[] = {(char*)"amplitron", (char*)"--preset",
                  (char*)"nonexistent_preset_file.json", (char*)"--headless"};
...
TEST(main_headless_routing_warnings_and_failures) {
  ...
  ASSERT_TRUE(ss_err.str().find("Could not find requested output device") !=
              std::string::npos);
  ...
  std::thread main_thread2(
      [&exit_code, &argv2]() { exit_code = app_main(8, argv2); });
  ...
  ASSERT_EQ(exit_code, 0);
  ASSERT_TRUE(ss_err2.str().find("Could not find requested input device") !=
              std::string::npos);
...
TEST(main_audio_failures) {
  ...
  ASSERT_EQ(exit_code, 1);
  ASSERT_TRUE(ss_err.str().find("Failed to initialize audio engine!") !=
              std::string::npos);

  // 2. Force start() to fail (warning only)
  g_mock_pa_initialize = []() -> PaError { return paNoError; };
  g_mock_pa_start_stream = [](PaStream*) -> PaError {
  ...
  char* argv2[] = {(char*)"amplitron", (char*)"--preset",
                   (char*)preset_path.c_str(), (char*)"--headless"};
  ...
  std::thread main_thread(
      [&exit_code, &argv2]() { exit_code = app_main(4, argv2); });
  ...
  ASSERT_EQ(exit_code, 0);
  ASSERT_TRUE(ss_err2.str().find("Warning: Could not start audio stream.") !=
              std::string::npos);
