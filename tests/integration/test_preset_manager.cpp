...
static bool write_wav_mono_pcm16(const std::string &path,
  ...
  const uint32_t data_bytes =
      static_cast<uint32_t>(samples.size()) * block_align;
...
TEST_F(PresetTest, save_creates_file) {
  ...
  bool ok =
      manager.save_preset(path, "Test Preset", "A test description", engine);
  ASSERT_TRUE(ok);
...
TEST_F(PresetTest, save_and_load_roundtrip) {
  ...
  [[maybe_unused]] bool orig_effect0_enabled =
      engine.effects()[0]->is_enabled();
...
TEST_F(PresetTest, midi_mappings_roundtrip) {
  ...
  bool saved = manager.save_preset(path, "Midi Test", "Testing midi mappings",
                                   engine, mappings);
  ASSERT_TRUE(saved);
...
TEST_F(PresetTest, parallel_amp_rig_integration) {
  ...
  int amp1 = graph.add_node(
      "Amp Sim", NodeRoutingType::StandardEffect,
...
TEST_F(PresetTest, save_graph_all_fields) {
  ...
  const char wav_header[46] = {
      'R', 'I', 'F', 'F', 38,         0,          0,   0,   'W', 'A',
      'V', 'E', 'f', 'm', 't',        ' ',        16,  0,   0,   0,
      1,   0,   1,   0,   (char)0x80, (char)0xbb, 0,   0,   0,   (char)0x77,
      1,   0,   2,   0,   16,         0,          'd', 'a', 't', 'a',
      2,   0,   0,   0,   0,          0};
...
TEST_F(PresetTest, manager_preset_to_json_string) {
  ...
  ASSERT_TRUE(std::any_of(
      parsed["nodes"].begin(), parsed["nodes"].end(), [](const auto &node) {
...
TEST(PresetManagerDirs, GetUserPresetsDirWellFormedOrEmpty) {
  ...
  ASSERT_TRUE(dir.empty() ||
              std::filesystem::path(dir).filename() == "presets");
}
...
TEST(PresetManagerDirs, GetPresetsDirUsesCustomDirAndNormalizesSlashes) {
  ...
  ASSERT_TRUE(resolved.find("test_custom_dir_with_slashes") !=
              std::string::npos);
...
TEST(PresetManagerDirs, GetPresetsDirFallsBackWhenUserDirUnavailable) {
  ...
  PresetManager::set_presets_dir("");
  std::string resolved = PresetManager::get_presets_dir();
  ASSERT_EQ(std::filesystem::path(resolved).filename().string(),
            std::string("presets"));
...
TEST(PresetJson, RoundtripWithMidiAndMetadata) {
  ...
  ASSERT_EQ(loaded.effects[0].metadata["ir_path"],
            std::string("path/to/ir.wav"));
...
TEST(PresetManagerIO, SavePresetDataToDirectoryFails) {
  ...
  ASSERT_FALSE(ok);
  ASSERT_TRUE(manager.get_last_error().find(
                  "Could not open file for writing") != std::string::npos);
}
...
TEST_F(PresetTest, AdvancedPresetManagerEdgeCases) {
  ...
  // Create a path containing backslashes (and double quotes on non-Windows
  // platforms)
#ifdef _WIN32
...
#else
  ASSERT_TRUE(config_content.find("presets\\\\escape_test_dir") !=
              std::string::npos);
#endif
...
TEST(PresetManagerIO, SaveFactoryPresetsUnreadableSource) {
  ...
  // Attempt to save factory presets; it should skip the unreadable bad_file
  // gracefully
