...
static void gui_recall_slot(SnapshotManager& mgr, int slot, AudioEngine& engine,
  ...
  history.execute(std::make_unique<RecallSnapshotCommand>(
      engine, before.effects, before.input_gain, before.output_gain,
...
TEST(snapshot_captures_effect_chain) {
  ...
  ASSERT_EQ(std::string(snap->effects[0].effect->name()),
...
TEST(snapshot_ab_switch_different_effect_chains) {
  ...
  ASSERT_EQ(std::string(engine.effects()[2]->name()),
...
TEST(snapshot_multiple_effects_all_params_preserved) {
  ...
  eq->params()[0].value =
      (eq->params()[0].min_val + eq->params()[0].max_val) / 2.0f;
