...
class TestSuite {
  ...
  int run(const std::string& junit_path = "", const std::string& filter = "") {
    ...
    for (auto& [name, fn] : tests_) {
      ...
      try {
        ...
        results_.push_back(
            {name, false, std::string("EXCEPTION: ") + e.what()});
      }
      if (!current_failed_) {
 ...
 private:
  ...
  void write_junit_xml(const std::string& filepath, int passed, int failed) {
    ...
    xml << "<testsuites tests=\"" << (passed + failed) << "\" failures=\""
        << failed << "\" errors=\"0\" name=\"AmplitronTests\">\n";
    xml << "  <testsuite name=\"AmplitronTestSuite\" tests=\""
        << (passed + failed) << "\" failures=\"" << failed
        << "\" errors=\"0\">\n";
    ...
    for (const auto& r : results_) {
      ...
      xml << "    <testcase name=\"" << escape_xml(test_name)
          << "\" classname=\"" << escape_xml(suite_name)
          << "\" time=\"0.0\">\n";
      if (!r.passed) {
        xml << "      <failure message=\"" << escape_xml(r.message)
            << "\" type=\"AssertionError\">" << escape_xml(r.message)
            << "</failure>\n";
      }
      xml << "    </testcase>\n";
    }
...
#define TEST_SINGLE(name)                                                \
  ...
  struct Register_##name {                                               \
    Register_##name() {                                                  \
      TestFramework::TestSuite::instance().add_test(#name, test_##name); \
    }                                                                    \
  } reg_##name;                                                          \
  }                                                                      \
  static void test_##name()
...
#define TEST(...) \
  TEST_EXPAND(    \
      TEST_GET_MACRO(__VA_ARGS__, TEST_SUITE_CASE, TEST_SINGLE)(__VA_ARGS__))
  ...
  struct Register_##FixtureName##_##TestName {                \
    Register_##FixtureName##_##TestName() {                   \
      TestFramework::TestSuite::instance().add_test(          \
          ...
          run_fixture_test_##FixtureName##_##TestName);       \
    }                                                         \
...
#define ASSERT_NE(a, b)                                                \
  do {                                                                 \
    ...
    if (_a == _b) {                                                    \
      std::ostringstream ss;                                           \
      ss << "ASSERT_NE failed: " #a " != " #b " (" << _a << ") (line " \
         << __LINE__ << ")";                                           \
      TestFramework::TestSuite::instance().fail(ss.str());             \
      return;                                                          \
    }                                                                  \
...
#define ASSERT_NEAR(a, b, eps)                                            \
  do {                                                                    \
    ...
    if (std::fabs(_a - _b) > (eps)) {                                     \
      std::ostringstream ss;                                              \
      ss << "ASSERT_NEAR failed: |" #a " - " #b "| <= " #eps " (" << _a   \
         << " vs " << _b << ", diff=" << std::fabs(_a - _b) << ") (line " \
         << __LINE__ << ")";                                              \
      TestFramework::TestSuite::instance().fail(ss.str());                \
      return;                                                             \
    }                                                                     \
...
#define ASSERT_THROW(expr, ExceptionType)                                  \
  do {                                                                     \
    ...
    if (!caught) {                                                         \
      std::ostringstream ss;                                               \
      ss << "ASSERT_THROW failed: " #expr " did not throw " #ExceptionType \
            " (line "                                                      \
         << __LINE__ << ")";                                               \
      TestFramework::TestSuite::instance().fail(ss.str());                 \
      return;                                                              \
