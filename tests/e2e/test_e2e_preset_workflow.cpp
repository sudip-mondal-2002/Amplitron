...
TEST_F(PresetTest, e2e_preset_workflow_roundtrip_and_processing) {
  ...
  if (!ng->params().empty()) {
    ng->params()[0].value =
        -30.0f;  // Set custom threshold within [-80.0f, 0.0f]
  }
  ...
  if (!dist->params().empty()) {
    dist->params()[0].value =
        8.5f;  // Set custom parameter within [1.0f, 20.0f]
  }
  ...
  // 3. Save the preset
  bool saved = manager.save_preset(path, "E2E Preset",
                                   "E2E Complete Signal Chain Test", engine);
  ASSERT_TRUE(saved);
  ...
  // Propagate the active sample rate to all newly deserialized nodes in the
  // graph
