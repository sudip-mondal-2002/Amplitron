 ...
 protected:
  ...
  void fill_sine(float freq, float amplitude = 1.0f) {
    for (int i = 0; i < BUFFER_SIZE; ++i) {
      input_buffer[i] =
          amplitude * std::sin(2.0f * 3.14159265f * freq * i / SR);
    }
  }
...
struct TestAccessor {
  ...
  static std::string& learn_effect_name(MidiManager& m) {
    return m.learn_effect_name_;
  }
  static std::string& learn_param_name(MidiManager& m) {
    return m.learn_param_name_;
  }
  static std::vector<MidiMapping>& mappings(MidiManager& m) {
    return m.mappings_;
  }
  ...
  static size_t get_queue_size(const MidiManager& mgr) {
    return mgr.midi_queue_.size();
  }
  ...
  static void commit_param_change(PedalWidget& w, int param_index,
                                  float old_val, float new_val) {
    w.commit_param_change(param_index, old_val, new_val);
  }
  static void render_footswitch_and_extras(PedalWidget& w, ImDrawList* dl,
                                           ImVec2 p0, ImVec2 p1,
                                           float pedal_width,
                                           float pedal_height, bool is_amp,
                                           bool enabled, bool& should_remove,
                                           float zoom) {
    w.render_footswitch_and_extras(dl, p0, p1, pedal_width, pedal_height,
                                   is_amp, enabled, should_remove, zoom);
  }
  ...
  static void render_knobs(PedalWidget& w, ImDrawList* dl, ImVec2 p0,
                           float pedal_width, bool is_amp, bool is_tuner,
                           bool is_ir_cab, float zoom) {
    w.render_knobs(dl, p0, pedal_width, is_amp, is_tuner, is_ir_cab, zoom);
  }
  static void render_amp_cabinet(PedalWidget& w, ImDrawList* dl, ImVec2 p0,
                                 ImVec2 p1, float pedal_width,
                                 float pedal_height, float zoom) {
    w.render_amp_cabinet(dl, p0, p1, pedal_width, pedal_height, zoom);
  }
  static void render_nam_loader_display(PedalWidget& w, ImVec2 p0,
                                        float pedal_width, float zoom) {
    w.render_nam_loader_display(p0, pedal_width, zoom);
  }
  static void render_standard_pedal(PedalWidget& w, ImDrawList* dl, ImVec2 p0,
                                    ImVec2 p1, float pedal_width, bool enabled,
                                    float zoom) {
