...
TEST(effect_color_lookup_known_effects) {
  const char* known[] = {"Distortion", "Overdrive",  "Delay",
                         "Reverb",     "Chorus",     "Equalizer",
                         "Noise Gate", "Compressor", "Cabinet"};
  ...
  for (const char* name : known) {
    ...
    ASSERT_GT(
        entry->pedal_color.x + entry->pedal_color.y + entry->pedal_color.z,
        0.0f);
    ASSERT_GT(entry->led_color.x + entry->led_color.y + entry->led_color.z,
              0.0f);
  }
}
...
TEST(effect_colors_are_distinct) {
  ...
  bool dist_od_same =
      (std::fabs(dist->pedal_color.x - od->pedal_color.x) < 0.01f &&
  ...
  bool dist_rv_same =
      (std::fabs(dist->pedal_color.x - rv->pedal_color.x) < 0.01f &&
