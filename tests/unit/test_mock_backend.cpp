...
TEST(AudioEngine_DeviceControlAndFailures) {
  ...
  // 2. set_input_device and set_output_device failures (when was_running is
  // false)
  ...
  // 3. set_input_device and set_output_device when running is true, but backend
  // set fails
  engine.start();
  mock->set_input_ok = false;
  ASSERT_FALSE(engine.set_input_device(0));
  ...
  // 4. Reversion failures (when was_running is true, device set succeeds, but
  // start fails, then revert fails)
  engine.start();
  mock->set_input_ok = true;
  mock->start_fail_count =
      2;  // fails first start, and second start during revert
  ASSERT_FALSE(engine.set_input_device(0));
...
TEST(AudioEngine_SetBufferSizeAndSampleRateReversion) {
  ...
  mock->start_fail_count =
      1;  // Fails the first start() call, reversion start() succeeds
  ...
  // 5. set_sample_rate: success when running with tuner tap and resizing
  // buffers on start
  ...
  // 6. set_sample_rate: failure/revert when running with tuner tap and graph
  // node
  mock->start_fail_count =
      1;  // Fails first start(), reversion start() succeeds
