...
Uint32 SDL_DequeueAudio(SDL_AudioDeviceID dev, void* data, Uint32 len) {
  (void)dev;
  Uint32 got =
      g_mock_sdl_queued_audio_size < len ? g_mock_sdl_queued_audio_size : len;
  if (got > 0) {
    if (data) {
...
TEST(SdlBackend_LifecycleAndCallback) {
  ...
  int bytes_len =
      512 * 2 * sizeof(float);  // 512 stereo frames = 1024 floats = 4096 bytes

  engine.process_audio_calls = 0;
  engine.last_input_data.clear();
  Amplitron::sdl_audio_callback(
      &backend, reinterpret_cast<Uint8*>(output_buffer.data()), bytes_len);
  ...
  SDL_QueueAudio(cap_dev, excess_input.data(),
                 excess_input.size() * sizeof(float));
  ...
  SDL_QueueAudio(cap_dev, short_input.data(),
                 short_input.size() * sizeof(float));
  ...
  // 4. Test callback when capture device is missing (cap_dev = 0, should memset
  // 0)
  backend.stop();
  ASSERT_EQ(backend.get_capture_device(), 0u);
  ...
  Amplitron::sdl_audio_callback(
      &backend_null, reinterpret_cast<Uint8*>(output_buffer_null.data()),
      bytes_len);
...
TEST(SdlBackend_FailurePathsAndDeviceSelection) {
  ...
  // 4. Preferred device detection (Case B: Devices with no matching keywords,
  ...
  // 5. Preferred device detection (Case C: Scarlett keyword matched)
  {
    ...
    g_mock_sdl_device_names = {"Internal Microphone", "Focusrite Scarlett 2i2",
                               "iRig Pro"};
    g_mock_sdl_last_opened_device = nullptr;
    ASSERT_TRUE(backend.start());
    ASSERT_NE(g_mock_sdl_last_opened_device, nullptr);
    ASSERT_EQ(std::string(g_mock_sdl_last_opened_device),
              "Focusrite Scarlett 2i2");
    backend.stop();
  }

  // 6. Capture device fails to open (does not block start(), but
  // capture_device_ is 0)
  {
    Amplitron::SdlBackend backend;
    ASSERT_TRUE(backend.initialize(&engine));
