...
TEST_F(PresetTest, session_manager_basic_lifecycle) {
  // 1. Construct SessionManager (fails back to relative paths if SDL is
  // uninitialized or headless)
...
TEST_F(PresetTest, session_manager_corrupted_load_handling) {
  ...
  // Let's verify it behaves deterministically (either returning empty/null or
  // throwing)
