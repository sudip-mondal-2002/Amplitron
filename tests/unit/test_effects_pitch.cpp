...
TEST_F(EffectsTest, octaver_sub_octave_produces_half_frequency) {
  ...
  for (int off = 0; off < WARM_UP; off += CHUNK) {
    for (int i = 0; i < CHUNK; ++i) {
      chunk_buf[i] =
          0.8f * std::sin(2.0f * 3.14159265f * FUND * (off + i) / SR);
    }
  ...
  for (int i = 0; i < N; ++i) {
    main_buf[i] =
        0.8f * std::sin(2.0f * 3.14159265f * FUND * (WARM_UP + i) / SR);
  }
...
TEST_F(EffectsTest, pitch_shifter_with_mix_and_shift_differs_from_dry) {
  ...
  const float mag_shifted =
      dft_magnitude_at(input_buffer, BUFFER_SIZE, shifted_freq);

  ASSERT_GT(mag_shifted, mag_440 * 0.5f);
}

TEST_F(EffectsTest,
       pitch_shifter_extreme_shift_wraps_phase_without_instability) {
  PitchShifter ps;
  ps.set_sample_rate(SR);
