...
TEST(ParameterChangeCommand_Execute) {
  ...
  CommandHistory history;
  history.execute(std::make_unique<ParameterChangeCommand>(test_engine(), fx, 0,
                                                           original, new_val));
...
TEST(ParameterChangeCommand_Coalescing) {
  ...
  history.push_executed(std::make_unique<ParameterChangeCommand>(
      test_engine(), fx, 0, original, original + 0.1f));
...
TEST(ParameterChangeCommand_NoCoalescing_DifferentParams) {
  ...
  history.push_executed(std::make_unique<ParameterChangeCommand>(
      test_engine(), fx, 0, orig0, orig0 + 0.5f));

  params[1].value = orig1 + 0.5f;
  history.push_executed(std::make_unique<ParameterChangeCommand>(
      test_engine(), fx, 1, orig1, orig1 + 0.5f));
...
TEST(LoadPresetCommand_UndoRedo) {
  ...
  history.push_executed(std::make_unique<LoadPresetCommand>(
      engine, std::move(before), before_in, before_out, std::move(after),
      after_in, after_out));
...
TEST(CommandHistory_RedoClearedOnNewAction) {
  ...
  CommandHistory history;
  history.execute(std::make_unique<AddEffectCommand>(
      engine, std::make_shared<Overdrive>()));
  history.execute(
      std::make_unique<AddEffectCommand>(engine, std::make_shared<Delay>()));
  ...
  history.execute(
      std::make_unique<AddEffectCommand>(engine, std::make_shared<Reverb>()));
...
TEST(CommandHistory_EdgeCases_And_Out_Of_Bounds) {
  ...
  // 2. ParameterChangeCommand out-of-bounds indices
  {
    ...
    auto cmd1 =
        std::make_unique<ParameterChangeCommand>(engine, fx, -1, 0.0f, 1.0f);
    auto cmd2 =
        std::make_unique<ParameterChangeCommand>(engine, fx, 999, 0.0f, 1.0f);
...
TEST(CommandHistory_PushExecutedCoalescing) {
  ...
  history.push_executed(std::make_unique<ParameterChangeCommand>(
      engine, fx, 0, original, original + 0.1f));
  history.push_executed(std::make_unique<ParameterChangeCommand>(
      engine, fx, 0, original + 0.1f, original + 0.2f));
...
TEST(ParameterChangeCommand_OutOfRangeParam) {
  ...
  auto cmd1 =
      std::make_unique<ParameterChangeCommand>(engine, fx, -5, 0.0f, 1.0f);
  auto cmd2 =
      std::make_unique<ParameterChangeCommand>(engine, fx, 9999, 0.0f, 1.0f);
