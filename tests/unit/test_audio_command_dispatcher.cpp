...
TEST(AudioCommandDispatcher_DrainCommands) {
  ...
  // - Valid effect_index lookup in dummy_effects, invalid param_index (out of
  // bounds)
  ...
  // 6. Unknown command / failure to lookup completely (effect_index out of
  // bounds for dummy_effects)
  dispatcher.push_param_change(99, 0, 1.0f);

  dispatcher.drain_commands(input_gain, output_gain, executor, main_graph,
                            dummy_effects);
