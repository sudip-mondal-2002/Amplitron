...
TEST(level_analyzer_normal_update_smoothing) {
  ...
  // Peak hold should match smoothed RMS initially as peak_hold_ - decay*dt will
  // be <= smoothed RMS
...
TEST(level_analyzer_invalid_inputs_nan_negative) {
  ...
  // Construct runtime bitwise NaN and Infinity to prevent compile-time
  // optimizations under -ffast-math
  float nan_val;
