...
static float tuner_detect_freq(float target_freq, int sample_rate = 48000) {
  ...
  for (int offset = 0; offset < total_samples; offset += chunk_size) {
    for (int i = 0; i < chunk_size; ++i) {
      buf[i] = 0.8f * std::sin(2.0f * 3.14159265f * target_freq * (offset + i) /
                               sample_rate);
    }
...
TEST(effect_factory_unknown_type_returns_nullptr) {
  ...
  auto effect2 =
      EffectFactory::instance().create_from_type("nonexistent_effect_type_xyz");
...
TEST(effect_factory_duplicate_registration_throws) {
  ...
  try {
    EffectFactory::instance().register_effect(
        "Overdrive", []() { return std::make_shared<Overdrive>(); });
