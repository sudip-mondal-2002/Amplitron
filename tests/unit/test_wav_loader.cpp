...
static bool write_pcm16_wav(const std::string& path, uint16_t num_channels,
...
static bool write_constant_mono_wav(const std::string& path, float value,
                                    int total_frames, uint32_t sample_rate) {
  const int16_t pcm = static_cast<int16_t>(
      std::lrint(std::fmax(-1.f, std::fmin(1.f, value)) * 32767.f));
